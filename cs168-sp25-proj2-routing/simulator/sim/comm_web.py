...
_base_path = os.path.join(
    os.path.dirname(os.path.realpath(sys.argv[0])), "../netvis/NetVis/"
)
            ...
            "entities": dict(
                [
                    (
                        ...
                        (
                            "circle"
                            if isinstance(n.entity, sim.api.HostEntity)
                            else "square"
                        ),
                    )
            ...
            while self._websocket_open:
                ...
                if not fin:
                    if op == self.WS_CONTINUE:
                        if old_op is None:
                            raise RuntimeError("Continuing unknown opcode")
                    else:
                        if old_op is not None:
                            raise RuntimeError("Discarded partial message")
                        old_op = op
                    data += d
                else:  # fin
                    if op == self.WS_CONTINUE:
                        if old_op is None:
                            raise RuntimeError("Can't continue unknown frame")
                        op = old_op
                    d = data + d
                    old_op = None
                    data = b""
...
class WebInterface(ThreadingMixIn, HTTPServer):
    def __init__(self):
        ...
        try:
            HTTPServer.__init__(
                self,
                (sim.config.remote_interface_address, sim.config.remote_interface_port),
                WebHandler,
            )
        except OSError as e:
            if e.errno == errno.EADDRINUSE:
                log.error(
                    "The webserver could not be started because the listening "
                    "port\nis already in use. "
        ...
        log.info("Webserver running at http://%s:%s", laddr[0], laddr[1])
