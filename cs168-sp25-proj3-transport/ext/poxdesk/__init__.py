...
class DispatcherSession(WebsocketHandler):
    ...
    @property
    def locked(self):
        if self.dispatcher.secret is None:
            return False
        return self._locked
    ...
    def _on_message(self, op, msg):
        ...
        try:
            ...
            if "object" in msg and msg["object"] is not None:
                ...
                if method is None:
                    ...
                    if method is None:
                        ...
                        if method is None:
                            method = getattr(
                                self.dispatcher, "_pdmethod_" + msg["method"]
                            )
            ...
            if (retf.called or errf.called) and (v is not None):
                log.error(
                    "Local method returned value, but also called return/error"
                    " function"
                )
            else:
                retf(v)
            ...
            if retf.called or errf.called:
                log.exception(
                    "Local method resulted in exception, but also called "
...
class Dispatcher(EventMixin):
    ...
    def websocket_enabled(self, value):
        if self._websocket_enabled is value:
            return
        if value is False:
            return  # Currently don't support disabling it
        self._websocket_enabled = True
        core.WebServer.set_handler(
            self.websocket_path, self.websocket_session_type, args=self
        )
...
class POXDesk(Dispatcher):
    def __init__(self, path, enable_websocket=False):
        ...
        self.path = path
        if enable_websocket:
            super(POXDesk, self).__init__(
                websocket_path="/poxdesk/_/ws/main",
                websocket_session_type=DispatcherSession,
            )
            if "ws" not in self.config:
                self.config["ws"] = {}
...
def ask_name():
    ...
    def _handle_NewSession(e):
        e.session.sendmsg(
            dict(method="promptBox", args=("What is your name?",), on_ret=log_it)
        )
...
def launch(websocket=False, secret_file=None, force_source=False):
    ...
    if force_source and have_qx:
        have_build = False

    if not have_qx and not have_build:
        log.error(
            "You need to have Qooxdoo in a directory named 'qx', or "
