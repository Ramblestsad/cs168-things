...
class Application(object):
    ...
    def __exec(self):
        ...
        self.popen = subprocess.Popen(
            self.cmdline, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True
        )
...
class Pox(Application):
    ...
    def __init__(self, test, tracefile):
        cmdline = "{0} config={1} tcpip.pcap --node=r1 --no-tx --filename={2}".format(
            self.POX_PY, test, tracefile
        )
...
class IndTestCase(unittest.TestCase):
    ...
    def run_pox(self, test, trace):
        ...
        self.assertTrue(
            self.pass_str in output,
            "Test failed, the console output was:\n{0}".format(output),
        )
...
class AutoGrader:
    ...
    def get_configs(self, test_path):
        cfg_file = lambda f: os.path.isfile(
    ...
    def get_tests_for_stage(self, stage):
        tests = [
            y
            for x, y in self.test_cases.items()
            if x.startswith("test_s{0}_".format(stage))
        ]
