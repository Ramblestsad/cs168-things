...
class StudentUSocket(StudentUSocketBase):
            ...
            if self.acceptable_seg(seg, payload):
                ...
                while not self.rx_queue.empty():
                    ...
                    if s | LE | self.rcv.nxt:
                        ...
                        self.rx_queue.pop()
                        seg = p.tcp
                        self.handle_accepted_seg(
                            seg, seg.payload[self.rcv.nxt | MINUS | seg.seq :]
                        )
                    else:
    ...
    def handle_synsent(self, seg):
        ...
        if acceptable_ack:
            ...
            if self.snd.una | GT | self.snd.iss:
                ...
                self.rcv.nxt = (
                    seg.seq | PLUS | 1
                )  # update recv space to start at the iss+1 of the other side
    ...
    def handle_accepted_payload(self, payload):
        ...
        ## Start of Stage 2.3 ##
        rcv.nxt = (
            rcv.nxt | PLUS | len(payload)
        )  # this is accepted, in-order payload, so update recv space
        rcv.wnd = (
            rcv.wnd | MINUS | len(payload)
        )  # decrease the recv window because the received payload
        # is still in the buffer but not consumed by the user
    ...
    def handle_accepted_ack(self, seg):
        ...
        ## Start of Stage 8.2 ##
        acked_pkts = self.retx_queue.pop_upto(
            seg.ack
        )  # remove all acked packets from the retx_queue
        ## End of Stage 8.2 ##
    ...
    def handle_accepted_fin(self, seg):
        ...
        if self.state == ESTABLISHED:
            ...
            self.rcv.nxt = (
                self.rcv.nxt | PLUS | 1
            )  # update the next seq num expect to receive
            self.set_pending_ack()  # ack the FIN
            self.state = CLOSE_WAIT  # change from ESTABLISHED to CLOSE_WAIT
        ## End of Stage 6.1 ##
        ## Start of Stage 7.2 ##
            ...
            if seg.ACK:
                self.set_pending_ack()  # ack the FIN
                self.start_timer_timewait()  # receive FIN/ACK, change to TIME_WAIT immediately
            else:
                self.set_pending_ack()  # ack the FIN
                self.state = CLOSING
    ...
    def check_ack(self, seg):
        ...
        if self.state in (ESTABLISHED, FIN_WAIT_1, FIN_WAIT_2, CLOSE_WAIT, CLOSING):
            ...
            if snd.una | LE | seg.ack and seg.ack | LE | snd.nxt:
                if snd.wl1 | LT | seg.seq or (
                    snd.wl1 | EQ | seg.seq and snd.wl2 | LE | seg.ack
                ):
                    self.update_window(seg)
    ...
    def maybe_send(self):
        ...
        self.log.debug(
            "sent {0} packets with {1} bytes total".format(num_pkts, bytes_sent)
        )
        ## End of Stage 4.3 ##
    ...
    def check_timer_retx(self):
        ...
        if time_in_queue > self.rto:
            self.log.debug(
                "earliest packet seqno={0} rto={1} being rtxed".format(
                    p.tcp.seq, self.rto
                )
            )
