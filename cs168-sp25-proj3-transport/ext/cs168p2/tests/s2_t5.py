...
def launch(log_name="s2_t4", data="", drop_count=0, run_time=5, server_isn=None):
    ...
    def setup():
        ...
        def do_score():
            tester.expect_eq(
                data.encode("ascii"), capp.rx_buffer, "payload correctly assembled"
            )
            num_pkts_payload = sum(1 for p in pkts if p.payload)
            tester.expect_true(
                6 <= num_pkts_payload <= 10, "[6,10] packets with payload"
            )
        ...
        class drop_one_pass_one(object):
            ...
            def __call__(self, dev, packet):
                ...
                if self.dropnext and tcp.seq not in self.dropped_pkts:
                    ...
                    self.dropnext = True
                    log.info(
                        "let packet through seq={0}, ack={1}".format(tcp.seq, tcp.ack)
                    )
                    return False
