...
def launch(log_name="test", server_isn=None):
    ...
    def setup():
        ...
        def do_score():
            ...
            tester.expect_true(
                pkts[0]._client and not pkts[0]._server, "first pkt comes from client"
            )
            tester.expect_true(
                pkts[0].SYN and not pkts[0].ACK, "first pkt is SYN not ACK"
            )

            # SYN+ACK from server
            server_seq = pkts[1].seq
            tester.expect_true(
                not pkts[1]._client and pkts[1]._server, "second pkt comes from server"
            )
            tester.expect_true(pkts[1].SYN and pkts[1].ACK, "second pkt is SYN+ACK")
            tester.expect_eq(
                client_seq | PLUS | 1, pkts[1].ack, "second pkt ack seq num is correct"
            )
            tester.expect_eq(
                SYN_SENT,
                pkts[1]._client_state,
            ...
            tester.expect_eq(
                server_seq | PLUS | 1, pkts[2].ack, "third pkt ack seq num is correct"
            )
            tester.expect_eq(
                ESTABLISHED,
                pkts[2]._client_state,
