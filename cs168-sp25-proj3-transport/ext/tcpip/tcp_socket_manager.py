...
class TCPSocketManager(object):
    ...
    def get_unused_port(self, ip):
        ...
        if ip == IP_ANY:
            names = set([(IP_ANY, p) for _, p in names])
    ...
    def _has_or_remove_socket(self, socket, remove):
        ...
        if s is socket:
            if remove:
                self.peered.pop((name, peer), None)
            return True
        s = self.unpeered.get(name)
        if s is socket:
            if remove:
                self.unpeered.pop(name, None)
            return True
        n = (IP_ANY, name[1])
        s = self.unpeered.get(n)
        if s is socket:
            if remove:
                self.unpeered.pop(n, None)
            return True
        return False
    ...
    def rx(self, dev, p):
        ...
        if s and s.state is tcp_sockets.LISTEN:
            return s.rx(p)
