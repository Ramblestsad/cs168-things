        ...
        try:
            while True:
                while self.sock.usock.state == tcp_sockets.ESTABLISHED:
                    ...
                    if not w:
                        break
                if self.sock.usock.state != tcp_sockets.ESTABLISHED:
                    break
...
class QuoteOfTheDay(SimpleReSocketApp):
    ...
    day = None
    default_quote = (
        "Your quote of the day: Today is a good day to install\n"
        "the 'fortune' command."
    )
    ...
    def fetch_quote():
        ...
        for _ in range(5):  # Three tries maximum
            p = subprocess.Popen((QuoteOfTheDay.quote_command,), stdout=subprocess.PIPE)
