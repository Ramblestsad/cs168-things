...
class Window(object):
    ...
    def check_accept(self, seg):
        ...
        if seg.len == 0 and self.wnd | MGT | 0:
            if (self.nxt | MLE | seg.seq) and (
                seg.seq | MLT | (self.nxt | PLUS | self.wnd)
            ):
                return True
            return False
        if seg.len > 0 and self.wnd == 0:
        ...
        if seg.len > 0 and self.wnd > 0:
            ...
            if (self.nxt | MLE | rhs) and (rhs | MLT | (self.nxt | PLUS | self.wnd)):
                return True
            return False
        return False
...
class Socket(object):
    ...
    def state(self, v):
        if v != self._state:
            ...
            for i in range(1, 5):
                fr = inspect.stack()[i]
                if fr[0].f_locals.get("self") is not self:
                    break
    ...
    def _new_packet(self, ack=True, data=None, syn=False):
        ...
        if self.state is LISTEN:
            pass
        elif self.state in (SYN_SENT, SYN_RECEIVED) and self.zero_initial_window:
            pass
        else:
        ...
        if data is None:
            self.log.info(
                "CRAFTED PACKET WITH ACK %s", self.rcv.nxt | MINUS | self.rcv.isn
            )  # XXX
        return p
    ...
    def _is_dup_ack(self, seg):
        ...
        if not seg.ACK:
            return False
        if not (self.tx_data or self.retx_queue):
            return False  # Either or only tx?
        if seg.payload:
            return False
        if seg.SYN or seg.FIN:
            return False
        if seg.ack | NE | self.snd.una:
            return False
        if self._read_win(seg) != self.snd.wnd:
            return False
        return True
    ...
    def connect(self, ip, port):
        ...
        if self.is_bound:
            ...
            if dev is None:
                raise PSError("No route to " + str(ip))
            if not dev.ip_addr:
                raise PSError("No IP")
    ...
    def recv(self, length=None, flags=0):
        ...
        if self._shut_rd:
            raise PSError("socket shut down for reading")
        ...
        if prv == 0 and cur != 0:
            ...
            if prv < split and cur >= split:
                ...
                if self._ack_pending == 0:
                    self._ack_pending = 1  # FIXME: this is ugly!
                self._maybe_send_pending_ack()
    ...
    def _maybe_send(self):
        ...
        def get_send_size(limited_tx=True):
            ...
            self.log.debug(
                "WND rwnd:%s cwnd:%s/%s wnd:%s flt:%s max:%s dupack:%s",
                self.snd.wnd,
                self.cwnd,
                cwnd,
                window_size,
                self.flight_size,
                max_size if max_size > 0 else 0,
                self._dup_ack_count,
            )  # XXX
            if max_size <= 0:
                return 0  # Already too much in flight
        ...
        if count:
            self.log.debug(
                "Sent %s packet(s) (%s payload bytes, %s remain)",
                count,
                total_size,
                len(self.tx_data),
            )
            self.log.info(
                "SENT TOT:%s  NEW:%s  FLT:%s BUF:%s",
                p.tcp.seq | MINUS | self.snd.isn,
                total_size,
                self.flight_size,
                len(self.tx_data),
            )
    ...
    def _maybe_retx(self, seqno=None):
        ...
        if from_timer:
            ...
            if self._retx_start + self.rto > self.stack.now:
        ...
        if from_timer:
            ...
            if maximum is None:
                maximum = 0xFFFFFFFF  # Lots and lots
            ...
            for i, p in enumerate(self.retx_queue):
                if p.tcp.seq | MGE | seqno:
                    if (p.tcp.seq | PLUS | tcplen(p.tcp)) | MGT | seqno:
                        self.log.info(
                            "Fast ReTX packet #%s (%s, %s)", i, p.tcp.seq, seqno
                        )
                        start_packet = i
                        break
            else:
                self.log.warn(
                    "No packet %i (%i) for fast retx",
                    seqno,
                    seqno | MINUS | self.snd.isn,
                )
                for i, p in enumerate(self.retx_queue):
                    self.log.warn(
                        "> %s %s", p.tcp.seq, p.tcp.seq | MINUS | self.snd.isn
                    )

        sent = 0
        for which in range(maximum):
            if (which + start_packet) >= len(self.retx_queue):
                break
            # now = self.stack.now
            ...
            if np.tcp.ACK:
                self._ts_last_ack = np.tcp.ack  # Kind of ugly to have here
            ...
            self.log.debug(
                "ReTX seq:%s len:%s rto:%s delta_time:%s",
                p.tcp.seq | MINUS | self.snd.isn,
    ...
    def _process_ack(self, ack):
        ...
        for p in self.retx_queue:
            ...
            if (seq | PLUS | length) | MLE | ack:
                ...
                self.log.warn(
                    "Segment partially ACKed (%s bytes of %s)",
                    acked_bytes,
                    len(seg.payload),
                )
        ...
        if old:
            self.retx_queue.pop_head(old)
        if old or len(self.retx_queue):
            self.log.debug(
                "Removed %s segment(s) from ReTX queue (%s remain)",
                old,
                len(self.retx_queue),
            )
    ...
    def rx(self, packet):
        ...
        * Reset the ZWP timer since we just got window info from the new packet
        ...
        if self._rx_one(packet):
            return

        while self.rx_queue and self.rx_queue[0].tcp.seq | LE | self.rcv.nxt:
            p = self.rx_queue.pop()
            self.log.debug(
                "RX queued packet (seq:%s nxt:%s)",
                p.tcp.seq | MINUS | self.rcv.isn,
                self.rcv.nxt | MINUS | self.rcv.isn,
            )
            if self._rx_one(p):
                return
    ...
    def _rx_syn_sent(self, packet):
        ...
        if packet.tcp.ACK:  # first
            if (
                packet.tcp.ack | MLE | self.snd.isn
                or packet.tcp.ack | MGT | self.snd.nxt
            ):
                if packet.tcp.RST:
                    return
                ...
                self._tx(rp)
                return
            if (self.snd.una | MLE | packet.tcp.ack) and (
                packet.tcp.ack | MLE | self.snd.nxt
            ):
                ack_ok = True
            else:
    ...
    def _rx_other(self, packet):
        ...
        if len(seg.payload) > 1:
            self.log.warn("GOT SEQ %s", seg.seq | MINUS | self.rcv.isn)
        ...
        if seg.seq | EQ | rcv.nxt:
            ...
            self._set_ack_pending()  # Send ACK per RFC 5681 p8
            self.log.debug(
                "Future packet queued for later (seq:%s nxt:%s)",
                seg.seq | MINUS | rcv.isn,
                rcv.nxt | MINUS | rcv.isn,
            )
            return
        ...
        if self.state in (ESTABLISHED, FIN_WAIT_1, FIN_WAIT_2, CLOSE_WAIT, CLOSING):
            ...
            if seg.ack | MGT | snd.nxt:
                ...
                self.log.info(
                    "Got duplicate ACK ack:%s (seq:%s) or ack:%s (seq:%s)",
                    seg.ack,
                    seg.seq,
                    seg.ack | MINUS | self.snd.isn,
                    seg.seq | MINUS | self.rcv.isn,
                )
                pass
                ...
                if self._is_dup_ack(seg):
                    ...
                    if not self._in_fast_recovery:
                        if self._dup_ack_count == 1:
                            ...
                            if (
                                seg.ACK and (seg.ack | MINUS | 1) | MGT | self._recover
                            ):  # RFC 6582 3.2
                                self._recover = self.snd.nxt | MINUS | 1
                                self._in_fast_recovery = True
                                # RFC 5681 3.2 (2) p9
                                self.ssthresh = (
                                    self.flight_size - self.limited_transmit_sent
                                ) / 2
                                ...
                                self.log.info(
                                    "FAST RETX %s %s",
                                    self.snd.una | MINUS | self.snd.isn,
                                    seg.ack | MINUS | self.snd.isn,
                                )
                                if not self._maybe_retx(self.snd.una):
                                    self.log.warn(
                                        "No retransmission in fast retransmit"
                                    )
                    else:
                        # RFC 5681 3.2 (4) p9
        ...
        if seg.FIN:
            ...
            self.log.debug(
                "Got FIN%s", "" if not payload else " on a packet with payload"
            )
            ...
            if self.rcv.nxt | EQ | got_fin_seq:
                ...
                self.log.warn(
                    "FIN seq isn't rcv.nxt (%s != %s) with payload size %s",
                    seg.seq,
                    self.rcv.nxt,
                    len(payload),
                )
    ...
    def _establish(self, packet):
        ...
        if self.parent:
            assert (
                not self in self.parent.accept_queue
            ...
            self.parent.log.debug(
                "Accept queue (size %s) got %s", len(self.parent.accept_queue), self
            )
    ...
    def _set_ack_pending(self, delayable=False):
        ...
        allowed by the RFC).
        """
        if self.use_delayed_acks is False:
            delayable = False
    ...
    def _acks_our_fin(self, ack):
        ...
        if self._fin_seqno is None:
            return False  # We haven't sent, so no!

        if ack | MGE | self._fin_seqno:
            return True
    ...
    def _set_fin_pending(self, next_state=None):
        ...
        if self._fin_sent and (next_state is not None):
            self.log.error(
                "Tried to set FIN pending with new state, but FIN has "
                "already been sent"
            )
    ...
    def _maybe_do_time_wait_timeout(self):
        ...
        Called from timer
        """
        if self._time_wait_ends_at is None:
            return
        if self._time_wait_ends_at > self.stack.now:
            return
    ...
    def _maybe_handle_zero_window(self):
        ...
        # be off.
        """
        if self.state in (LISTEN, CLOSED):
            return
        if self.rcv.wnd != 0:
        ...
        if self.stack.now < self._zwp_at:
    ...
    def _maybe_update_rto(self, seg):
        ...
        """
        if not seg.ACK:
            return  # Couldn't be responding to a sample!
        ...
        for p in self.retx_queue:
            if seg.ack | MGT | p.tcp.seq:
                if seg.ack | MLE | (p.tcp.seq | PLUS | tcplen(p.tcp)):
                    ...
                    self.log.debug(
                        "Maybe using packet to update RTO (rack:%s ack:%s "
                        "una:%s nxt:%s)",
                        seg.ack | MINUS | self.snd.isn,
                        seg.ack,
                        self.snd.una,
        ...
        self.log.debug(
            "Not using packet to update RTO (rack:%s)", seg.ack | MINUS | self.snd.isn
        )  # XXX
    ...
    def _back_off_rto(self):
        ...
        self.log.warn(
            "RTO BACKOFF now %0.3f (was:%0.3f - R:--- SRTT:%s RTTVAR:%s)"
    ...
    def cwnd(self, value):
        if value != self.cwnd:
            self.log.debug(
                "CWND CHANGE %s -> %s (flgt:%s)", self._cwnd, value, self.flight_size
            )  # XXX
        if value < 0:
    ...
    def _on_unacked_data_acked(self, seg):
        ...
        if self._in_fast_recovery:
            ...
            if seg.ack | MGT | self._recover:
                ...
                # Deflate the window
                self.cwnd = min(
                    self.ssthresh, max(self.flight_size, self.smss) + self.smss
                )
                self._exit_frr()
            else:
                # Partial acknowledgement
                self.log.debug(
                    "FRR Partial ACK; retx rseq:%s seq:%s",
                    self.snd.una | MINUS | self.snd.isn,
                    self.snd.una,
                )  # XXX
                if not self._maybe_retx(self.snd.una):
        ...
        if self.in_slow_start:
            ...
            # XXX Uncomment this log!
            self.log.debug(
                "SS CWND %s -> %s (acked_bytes:%s smss:%s)",
                old_cwnd,
                self.cwnd,
                acked_bytes,
                self.smss,
    ...
    def _process_timestamp(self, seg):
        ...
        if tsech != 0:
            ...
            if ts_update:
                # Sloppy log message...
                self.log.debug(
                    "Maybe using TS to update RTO (rack:%s ack:%s una:%s" " nxt:%s)",
                    seg.ack | MINUS | self.snd.isn,
                    seg.ack,
                    self.snd.una,
                    self.snd.nxt,
