...
class SimNetDev(NetDev):
    ...
    def send(self, packet, gw):
        ...
        if queue is None:
            ...
            return
        if self.tx_capture_proc:
            self.tx_capture_proc(self, False, raw, ip)
    ...
    def rx(self, packet, src):
        ...
        if self.stack:
            ...
            p.ipv4 = packet
            p.break_payload()
            if self.rx_capture_proc:
                self.rx_capture_proc(self, True, None, packet)
...
class TopologyBase(object):
        ...
        def share_routes(srcdev, dstdev, wire):
            ...
            for r in routes:
                ...
                if not r2 or (r2 and r2[0].metric > metric):
                    if r.size == 32 and r.prefix == dstdev.ip_addr:
                        continue
    ...
    def set_queue(self, n1, n2, factory1=None, factory2=None):
        ...
        if isinstance(n1, str):
            src, dst = self.get_node(src), self.get_node(dst)
        if isinstance(n1, Node):
            n1, n2 = self.get_devs(n1, n2)
        ...
        if factory1 is not False:
            if not isinstance(factory1, Queue):
                factory1 = factory1(n1, n2)
            self.queues.pop((n1, n2), None)  # Is this necessary?
            self.queues[n1, n2] = factory1
        if factory2 is not False:
            if not isinstance(factory2, Queue):
                factory2 = factory2(n2, n1)
    ...
    def get_queue(self, src, dst):
        ...
        if isinstance(src, Node):
            src, dst = self.get_devs(src, dst)
        if (src, dst) not in self.queues:
            log.warn(
                "Using default queue for %s<->%s (you should probably specify "
                "this yourself!)",
                src,
                dst,
            )
