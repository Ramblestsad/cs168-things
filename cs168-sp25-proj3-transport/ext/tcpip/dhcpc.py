...
class DHCPClient(DHCPClientBase):
    ...
    def __init__(self, netdev):
        ...
        super(DHCPClient, self).__init__(
            port_eth=eth, total_timeout=100, auto_accept=True, name=name
        )
    ...
    def _check_done(self):
        ...
        if self.bound.routers:
            ...
            if self.add_default_route:
                r = self.netdev.stack.add_route(
                    prefix="0.0.0.0/0", gw=gw, dev=self.netdev
                )
    ...
    def _sniff(self, packet):
        if self._check_done():
            return True  # Remove sniffer
        if packet.rx_dev is not self.netdev:
            return
