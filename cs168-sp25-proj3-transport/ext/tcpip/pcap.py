...
def _add_pcap(node, rx, tx):
    ...
    for dev in node.stack.netdevs.values():
        ...
        if not isinstance(dev, SimNetDev):
            continue
        if not isinstance(dev.dst_dev, SimNetDev):
            continue
        if tx:
...
def launch(
    filename=None, nodes=None, node=None, no_tx=False, no_rx=False, __INSTANCE__=None
):
    if node and nodes:
        nodes += "," + node
    elif node:
