...
class RecocoSocket(object):
    ...
    def _new(self, usock=None):
        return type(self)(self.stack, usock=usock, connect_timeout=self.connect_timeout)
...
class SimpleReSocketApp(Task):
    ...
    def __init__(
        ...
        if listen is False and parent is None:
            if ip == "0.0.0.0":
                raise RuntimeError("Must set IP address")
        self.start()
    ...
    def _on_connected(self):
        ...
        """
        self.log.error("Unimplemented connection handler for %s", type(self).__name__)
    ...
    def run(self):
        if self.parent:
            ...
            try:
                ...
                self.log.debug(
                    "%s connected to %s:%s", type(self).__name__, self.ip, self.port
                )
            except ReSocketError as e:
...
class FastSender(SimpleReSocketApp):
    ...
    def __init__(
        ...
        super(FastSender, self).__init__(
            socket=socket,
            ip=ip,
            port=port,
            listen=listen,
