...
class NormalL2Dev(L2NetDev):
    def send(self, packet, gw):
        ...
        self.stack.arp_table.send(
            e, router_ip=next_hop, src_ip=self.ip_addr, send_function=self.send_raw_l2
        )
...
class TunTapDev(NetDev):
    def _init(self, dev_name, tun=False, ip_addr=None):
        ...
        self.iface = TapInterface(dev_name, tun, raw=raw, protocol=pkt.ethernet.IP_TYPE)
...
class TunDev(TunTapDev):
    ...
    def _handle_RXData(self, e):
        if self.stack:
            if e.interface.last_protocol != pkt.ethernet.IP_TYPE:
                return
            p = self.stack.new_packet()
            p.rx_dev = self
