...
import struct
from .recoco_sockets import (
    RecocoSocketManager,
    DataLogger,
    FastSender,
    BasicStateTransServer,
)
...
class Node(object):
    ...
    def __init__(self, name):
        ...
        self.new_usocket = lambda node: tcp_sockets.Socket(node.stack.socket_manager)
    ...
    def resocket(self, domain=socket.AF_INET, type=socket.SOCK_STREAM):
        ...
        if self.recoco_socket_manager is None:
            ...
            self.recoco_socket_manager = RecocoSocketManager(
                self.stack, usocket_factory=f
            )
    ...
    def _new_resocket_app(self, app, ip=None, port=None, listen=False, delay=0, **kw):
        if listen is False:
    ...
    def new_data_logger(self, ip=None, port=None, listen=False, delay=0):
        return self._new_resocket_app(
            DataLogger, ip=ip, port=port, listen=listen, delay=delay
        )

    def new_echo(self, ip=None, port=None, listen=False, delay=0):
        return self._new_resocket_app(
            Echo, ip=ip, port=port, listen=listen, delay=delay
        )

    def new_fast_sender(self, data, delay=0, ip=None, port=None, listen=False):
        return self._new_resocket_app(
            FastSender, bytes=data, ip=ip, port=port, listen=listen, delay=delay
        )

    def new_basic_state_trans(self, ip=None, port=None, listen=False, delay=0):
        return self._new_resocket_app(
            BasicStateTransServer, ip=ip, port=port, listen=listen, delay=delay
        )

    def nc(self, ip=None, port=None, listen=False, no_console=False, **kw):
        return self._new_resocket_app(
            NetCat, ip=ip, port=port, listen=listen, no_console=no_console, **kw
        )
    ...
    def ping(self, ip, count=1, interval=1, ttl=63, size=56, df=False):
        ...
        def send_ping(c, seq, eid=None):
            ...
            if df:
                ipp.flags = ipp.flags | ipp.DF_FLAG
            ipp.payload = icmpp
...
class NetCat(recoco_sockets.SimpleReSocketApp):
    ...
    def _on_connected(self):
        if self.send_data is not None:
            self.log.info(
                "NetCat connected.  Sending %s bytes of data." % len(self.send_data)
            )
            ...
            self.log.info(
                "NetCat connected.  Enter '%s' on an empty line to exit.",
                self.end_sequence,
            )
        if self.raw_mode:
            while True:
