...
class MasqTable(object):
    ...
    def get_entry_out(self, p, out_dev):
        ...
        """
        if p.ipv4 is None or p.tcp is None:
            return None
...
class Route(object):
    ...
    def __str__(self):
        ...
        return "%-20s %-17s %6s %-20s" % (p, self.real_gw, self.metric, self.dev_name)
...
class Packet(object):
    ...
    def app(self, data):
        if not isinstance(data, bytes):
            data = data.pack()
        if self.tcp:
            self.tcp.payload = data
        elif self.udp:
            self.udp.payload = data
        else:
        ...
        try:
            ...
            while True:
                ...
                if len(data) > 0xFFFF:
                    self.failed = True
                    return
                if nxt.ipv4.flags & nxt.ipv4.MF_FLAG:
                    continue
                break
...
class IPStack(object):
    ...
    def add_netdev(self, netdev):
        if netdev.stack:
            raise RuntimeError("NetDev already has IP stack")
    ...
    def _rx_arp_request(self, packet, arpp):
        ...
        if arpp.opcode != arpp.REQUEST:
            return False
        if arpp.hwtype != arpp.HW_TYPE_ETHERNET:
            return False
        if arpp.prototype != arpp.PROTO_TYPE_IP:
            return False
        if arpp.hwlen != 6:
            return False
        if arpp.protolen != 4:
            return False
        if not netdev.has_ip_addr(arpp.protodst):
            return False

        self.arp_table.rx_arp(arpp)

        # TODO: proxy ARP / respond for other interfaces?
        if netdev.ip_addr != arpp.protodst:
            return False
    ...
    def add_packet_capture(
        self, devs, handler, rx=True, tx=False, ip_only=False, eth_only=False
    ):
        """
        ...
        if isinstance(devs, str):
            devs = self.get_netdevs(devs)

        result = []

        for dev in devs:
            if dev.is_l2 and not cap_eth:
                continue
            if not dev.is_l2 and not cap_ip:
                continue
            if rx:
    ...
    def rx(self, p):
        if p.rx_dev is not None and p.rx_dev.enable_rx is False:
            return
        ...
        if dead_snoop:
            self.log.debug(
                "%s self-removing packet sniffer(s) removed", len(dead_snoop)
            )
        ...
        if p.ipv4:
            ...
            if self.enable_ip_forward:
                if not p.rx_dev.enable_ip_forward_from:
                    return  # TODO: ICMP?
                ...
                if not out_dev.enable_ip_forward_to:
                    return  # TODO: ICMP?
        ...
        if len(p) > out_dev.mtu:
            ...
            if p.ipv4.flags and p.ipv4.DF_FLAG:
                ...
                if p.ipv4.protocol == p.ipv4.ICMP_PROTOCOL:
                    if p.ipv4.find("icmp").type == pkt.TYPE_DEST_UNREACH:
                        return
                payload = p.ipv4.pack()
    ...
    def _local_rx_icmp(self, dev, p):
        if p.icmp.type == pkt.TYPE_ECHO_REQUEST:
            ...
            self.send(rp)
        elif p.icmp.type == pkt.TYPE_ECHO_REPLY and isinstance(
            p.icmp.payload, pkt.echo
        ):
            ...
            if payload.startswith("PXIP"):
                ...
                try:
                    ...
                    pass
            self.log.info(
                "Pong %s->%s seq:%s bytes:%s ttl:%s time:%sms",
                p.ipv4.srcip,
                p.ipv4.dstip,
                p.icmp.payload.seq,
                len(p.icmp.payload.payload),
                p.ipv4.ttl,
                delay,
            )
        else:
