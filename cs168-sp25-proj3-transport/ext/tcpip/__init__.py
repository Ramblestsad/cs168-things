...
def data_logger(node, port=0, ip=None, listen=False, delay=None, __INSTANCE__=None):
    if delay is None:
        delay = 0 if listen else 1
    else:
        delay = float(delay)
    _new_app(node, "new_data_logger", port, ip, listen, delay=delay)


def basic_state_trans(
    node, port=0, ip=None, listen=True, delay=None, __INSTANCE__=None
):
    if delay is None:
        delay = 0 if listen else 1
    else:
...
def fast_sender(node, bytes, delay=0, port=0, ip=None, listen=False, __INSTANCE__=None):
    bytes = int(bytes)
    delay = float(delay)
...
def realworld_test(
    tap=None, tun=None, pcap=None, ip_addr=None, dhcp=False, masq=False, local_eth=None
):
    t = RealtimeTopology()
    if _default_queue_type:
        t.default_queue_factory = t.make_factory(_default_queue_type)
...
def random_loss(loss, node1="r1", node2="r2", unidirectional=False, __INSTANCE__=None):
    loss = float(loss)
...
def set_ip(node, ip, dev=None, devpat=None, first=False, __INSTANCE__=None):
    ...
    if len(matching) > 1 and not first:
        ...
        raise RuntimeError("More than one device matching '%s': %s" % (dev, names))
    device = matching[0]
    log.info("Changing IP address of %s from %s to %s", device.name, device.ip_addr, ip)
    device.ip_addr = ip
...
def quit_at(time):
    ...
    def quit():
        core.getLogger().info(
            "Quitting at requested time (%s)", units.seconds_to_str(time)
        )
        core.quit()
...
class SimTimeFilter(logging.Filter):
    ...
    def filter(self, record):
        if not self.time_manager:
            if not core.hasComponent("sim_topo"):
                return True
            self.time_manager = core.sim_topo

        record.msg = (
            "[" + units.seconds_to_str(self.time_manager.now) + "] " + record.msg
        )
        return True
