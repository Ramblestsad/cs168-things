...
class l3_switch(EventMixin):
    ...
    def _send_lost_buffers(self, dpid, ipaddr, macaddr, port):
        ...
        if (dpid, ipaddr) in self.lost_buffers:
            ...
            log.debug(
                "Sending %i buffered packets to %s from %s"
    ...
    def _handle_openflow_PacketIn(self, event):
        ...
        if isinstance(packet.next, ipv4):
            log.debug(
                "%i %i IP %s => %s", dpid, inport, packet.next.srcip, packet.next.dstip
            )
            ...
            if dstaddr in self.arpTable[dpid]:
                ...
                self.outstanding_arps = {
                    k: v for k, v in self.outstanding_arps.items() if v > time.time()
                }
                ...
                e = ethernet(
                    type=ethernet.ARP_TYPE, src=packet.src, dst=ETHER_BROADCAST
                )
                e.set_payload(r)
                log.debug(
                    ...
                    if a.protosrc != 0:
                        ...
                        if a.protosrc in self.arpTable[dpid]:
                            if self.arpTable[dpid][a.protosrc] != (inport, packet.src):
                                log.info(
                                    "%i %i RE-learned %s", dpid, inport, a.protosrc
                                )
                                if self.wide:
                                    # Make sure we don't have any entries with the old info...
                                    msg = of.ofp_flow_mod(command=of.OFPFC_DELETE)
