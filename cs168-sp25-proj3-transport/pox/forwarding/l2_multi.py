...
def _get_raw_path(src, dst):
    ...
    if intermediate is None:
        ...
        return []
    return (
        _get_raw_path(src, intermediate)
        + [intermediate]
...
class WaitingPath(object):
    ...
    def notify(self, event):
        ...
        if len(self.xids) == 0:
            ...
            if self.packet:
                log.debug(
                    "Sending delayed packet out %s" % (dpid_to_str(self.first_switch),)
                )
                msg = of.ofp_packet_out(
                    data=self.packet, action=of.ofp_action_output(port=of.OFPP_TABLE)
                )
...
class Switch(EventMixin):
    ...
    def install_path(self, dst_sw, last_port, match, event):
        ...
        log.debug(
            "Installing path for %s -> %s %04x (%i hops)",
            match.dl_src,
            match.dl_dst,
            match.dl_type,
            len(p),
