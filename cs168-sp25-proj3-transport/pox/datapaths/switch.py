...
class SoftwareSwitchBase(object):
    def __init__(
        ...
        for value, name in ofp_action_type_map.items():
            ...
            if not h:
                continue
            if getattr(self.features, "act_" + name) is False:
                continue
        ...
        if event.reason in (OFPRR_IDLE_TIMEOUT, OFPRR_HARD_TIMEOUT, OFPRR_DELETE):
            ...
            for entry in event.removed:
                if entry.flags & OFPFF_SEND_FLOW_REM and not entry.flags & OFPFF_EMERG:
                    ...
                    count += 1
            self.log.debug(
                "%d flows removed (%d removal notifications)", len(event.removed), count
            )
    ...
    def _rx_packet_out(self, packet_out, connection):
        ...
        if packet_out.data:
            self._process_actions_for_packet(
                packet_out.actions, packet_out.data, packet_out.in_port, packet_out
            )
            ...
            self.log.warn(
                "packet_out: No data and no buffer_id -- " "don't know what to send"
            )
    ...
    def send_hello(self, force=False):
        ...
        if self._has_sent_hello and not force:
            return
    ...
    def send_packet_in(
        self, in_port, buffer_id=None, packet=b"", reason=None, data_length=None
    ):
        """
        Send PacketIn
        """
        ...
        msg = ofp_packet_in(
            xid=0, in_port=in_port, buffer_id=buffer_id, reason=reason, data=packet
        )
    ...
    def _output_packet(self, packet, out_port, in_port, max_len=None):
        ...
        def real_send(port_no, allow_in_port=False):
            ...
            if self.ports[port_no].config & OFPPC_NO_FWD:
                self.log.warn(
                    "Dropping packet sent on port %i: Forwarding disabled", port_no
                )
                return
        ...
        if out_port < OFPP_MAX:
            ...
            self.send_packet_in(
                in_port, buffer_id, packet, reason=OFPR_ACTION, data_length=max_len
            )
    ...
    def _process_actions_for_packet_from_buffer(self, actions, buffer_id, ofp=None):
        """
    ...
    def _flow_mod_delete(self, flow_mod, connection, table, strict=False):
        ...
        if out_port == OFPP_NONE:
            out_port = None  # Don't filter
        table.remove_matching_entries(
            match,
            priority=priority,
            strict=strict,
            out_port=out_port,
...
class ExpireMixin(object):
    ...
    def __init__(self, *args, **kw):
        ...
        self._expire_timer = Timer(
            expire_period, self.table.remove_expired_entries, recurring=True
        )
...
class OFConnection(object):
    ...
    def _error_handler(self, reason, info):
        ...
        if reason == OFConnection.ERR_BAD_VERSION:
            ...
            self.log.error(
                "Different idea of message length for %s "
...
class SwitchFeatures(object):
    ...
    def capability_bits(self):
        ...
        """
        return sum((v if getattr(self, k) else 0) for k, v in self._cap_info.items())
    ...
    def action_bits(self):
        ...
        """
        return sum(
            (1 << v if getattr(self, k) else 0) for k, v in self._act_info.items()
        )
