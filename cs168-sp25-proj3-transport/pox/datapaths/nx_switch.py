...
_slave_blacklist = set(
    [of.ofp_flow_mod, of.ofp_packet_out, of.ofp_port_mod, of.ofp_barrier_request]
)
...
class NXSoftwareSwitch(SoftwareSwitch):
    ...
    def rx_message(self, connection, msg):
        ...
        if not self.check_rights(msg, connection):
            self.log.warn(
                "Message %s not allowed for slave controller %d", msg, connection.ID
            )
        ...
        if type(message) in _messages_for_all:
            ...
            if len(masters) > 0:
                ...
                if len(others) > 0:
                    ...
                    self.log.info(
                        "Could not find any connection to send messages %s",
                        str(message),
                    )
