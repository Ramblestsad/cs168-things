...
class VirtualPort(object):
    ...
    def inject(self, data):
        ...
        if self.phy.state & of.OFPPS_LINK_DOWN:
            return
        ...
        for p in self._patchbay[self.channel]:
            ...
            if not p.started:
                continue
            if p.phy.state & of.OFPPS_LINK_DOWN:
                continue
            log.debug(
                "%s.%s -> %s.%s",
                self.switch.name,
...
class PCapSwitch(ExpireMixin, SoftwareSwitchBase):
    ...
    def __init__(self, **kw):
        ...
        self.magic_virtual_port_names = kw.pop(
            "magic_virtual_port_names", self.magic_virtual_port_names
        )
    ...
    def add_interface(
        self, name, port_no=-1, on_error=None, start=False, virtual=False
    ):
        """
        Add an interface
