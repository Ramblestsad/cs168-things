...
class LLDPSender(object):
    ...
    def del_port(self, dpid, port_num, set_timer=True):
        if port_num > of.OFPP_MAX:
            return
        self._this_cycle = [
            p for p in self._this_cycle if p.dpid != dpid or p.port_num != port_num
        ]
        self._next_cycle = [
            p for p in self._next_cycle if p.dpid != dpid or p.port_num != port_num
        ]
        if set_timer:
            self._set_timer()
...
class Link(namedtuple("LinkBase", ("dpid1", "port1", "dpid2", "port2"))):
    ...
    def __repr__(self):
        return "Link(dpid1=%s,port1=%s, dpid2=%s,port2=%s)" % (
            self.dpid1,
            self.port1,
            self.dpid2,
            self.port2,
...
class Discovery(EventMixin):
    ...
    def __init__(
        ...
        core.listen_to_dependencies(
            self, listen_args={"openflow": {"priority": 0xFFFFFFFF}}
        )
    ...
    def install_flow(self, con_or_dpid, priority=None):
        ...
        match = of.ofp_match(
            dl_type=pkt.ethernet.LLDP_TYPE, dl_dst=pkt.ETHERNET.NDP_MULTICAST
        )
        msg = of.ofp_flow_mod()
    ...
    def _handle_openflow_PacketIn(self, event):
        ...
        if (
            packet.effective_ethertype != pkt.ethernet.LLDP_TYPE
        ...
        if originatorPort is None:
            log.warning(
                "Thought we found a DPID, but port number didn't " + "make sense"
            )
            return EventHalt
...
class DiscoveryGraph(object):
    ...
    def __init__(self, auto_export_file=None, use_names=None, auto_export_interval=2.0):
    ...
    def _auto_export_interval(self):
        if self.auto_export_interval:
            core.call_delayed(self.auto_export_interval, self._auto_export_interval)
...
def graph(export=None, dpids_only=False, interval="2.0"):
    ...
    core.registerNew(
        DiscoveryGraph,
        export,
        use_names=not dpids_only,
        auto_export_interval=float(interval),
    )


def launch(
    no_flow=False, explicit_drop=True, link_timeout=None, eat_early_packets=False
):
