...
class nicira_base(ofp_vendor_base):
    ...
    def __eq__(self, other):
        if type(self) != type(other):
            return False
        if not ofp_vendor_base.__eq__(self, other):
            return False
...
class nx_flow_mod(of.ofp_flow_mod, of.ofp_vendor_base):
    ...
    def unpack(self, raw, offset=0):
        ...
        offset, self.actions = of._unpack_actions(raw, length - (offset - _o), offset)
...
class nx_action_bundle(of.ofp_action_vendor_base):
    ...
    def _eq(self, other):
        ...
        if self.basis != other.basis:
            return False
        if self.slave_type != other.slave_type:
            return False
        if self.slaves != other.slaves:
            return False
    ...
    def _pack_body(self):
        p = struct.pack("!HHHH", self.subtype, self.algorithm, self.fields, self.basis)
    ...
    def _show(self, prefix):
        s = ""
        for f in (
            "subtype algorithm fields basis slave_type nbits offset " "dst"
        ).split():
...
class nx_output_reg(of.ofp_action_vendor_base):
    ...
    def _unpack_body(self, raw, offset, avail):
        offset, (self.subtype, ofs_nbits, reg, self.max_len, _, _) = of._unpack(
            "!HH4sHHI", raw, offset
        )
...
class nx_reg_move(of.ofp_action_vendor_base):
    ...
    def _eq(self, other):
        ...
        if self.dst != other.dst:
            return False
        if self.dst_ofs != other.dst_ofs:
            return False
        if self.src != other.src:
            return False
        if self.src_ofs != other.src_ofs:
            return False
        return True
    ...
    def _pack_body(self):
        ...
        p = struct.pack(
            "!HHHH4s4s", self.subtype, self.nbits, self.src_ofs, self.dst_ofs, src, dst
        )
        return p
...
class nx_reg_load(of.ofp_action_vendor_base):
    ...
    def _unpack_body(self, raw, offset, avail):
        offset, (self.subtype, ofs_nbits, dst, self.value) = of._unpack(
            "!HH4sQ", raw, offset
        )
...
class nx_action_controller(of.ofp_action_vendor_base):
    ...
    def _pack_body(self):
        p = struct.pack(
            "!HHHB", self.subtype, self.max_len, self.controller_id, self.reason
        )
        p += of._PAD
        return p
...
class nx_action_push_mpls(of.ofp_action_vendor_base):
    ...
    def _eq(self, other):
        if self.subtype != other.subtype:
            return False
        if self.ethertype != other.ethertype:
            return False
        return True
...
class nx_action_resubmit(of.ofp_action_vendor_base):
    ...
    def resubmit_table(cls, table=255, in_port=of.OFPP_IN_PORT):
        return cls(subtype=NXAST_RESUBMIT_TABLE, in_port=in_port, table=table)
    ...
    def _unpack_body(self, raw, offset, avail):
        offset, (self.subtype, self.in_port, self.table) = of._unpack(
            "!HHB", raw, offset
        )
...
class nx_action_fin_timeout(of.ofp_action_vendor_base):
    ...
    def _eq(self, other):
        if self.subtype != other.subtype:
            return False
        if self.fin_idle_timeout != other.fin_idle_timeout:
            return False
        if self.fin_hard_timeout != other.fin_hard_timeout:
            return False
        return True

    def _pack_body(self):
        p = struct.pack(
            "!HHH", self.subtype, self.fin_idle_timeout, self.fin_hard_timeout
        )
        p += of._PAD2
        return p
...
class nx_action_learn(of.ofp_action_vendor_base):
    ...
    def _unpack_body(self, raw, offset, avail):
        ...
        offset, (
            ...
            self.fin_idle_timeout,
            self.fin_hard_timeout,
        ) = of._unpack("!HHHHQHBBHH", raw, offset)
    ...
    def _show(self, prefix):
        s = ""
        ff = (
            "idle_timeout hard_timeout priority cookie flags table_id "
...
class flow_mod_spec(object):
        ...
        if n_bits is None:
            ...
            if n_bits is None:
                ...
                if dst.n_bits is not None and dst.n_bits > n_bits:
                    raise RuntimeError(
                        "dst n_bits greater than source n_bits "
    ...
    def unpack_new(cls, raw, offset=0):
        ...
        offset, src = nx_learn_spec_src.unpack_subclass(
            (v >> 13) & 1, n_bits, raw, offset
        )
        offset, dst = nx_learn_spec_dst.unpack_subclass(
            (v >> 11) & 3, n_bits, raw, offset
        )
...
class nxm_entry(object):
    ...
    def __eq__(self, other):
        if type(self) != type(other):
            return False
        if self._nxm_type != other._nxm_type:
            return False
        if self.value != other.value:
            return False
        if self.mask != other.mask:
            return False
        if self.is_reg != other.is_reg:
            return False
        return True
    ...
    def pack(self, omittable=False, header_only=False):
        ...
        if mask is not None:
            assert 0 == sum(
                v & (0xFF & ~m) for v, m in zip(value, mask)
            ), "nonzero masked bits"
            r += mask
...
def _make_nxm(__name, __vendor, __field, __len=None, type=None, **kw):
    """
    ...
    kw["_nxm_type"] = t
    if __len is not None:
        kw["_nxm_length"] = __len
    import builtins
...
class nx_async_config(nicira_base):
    ...
    def set_packet_in(self, bit, master=True, slave=True):
        if master:
            self.packet_in_mask |= bit
        if slave:
            self.packet_in_mask_slave |= bit

    def set_port_status(self, bit, master=True, slave=True):
        if master:
            self.port_status_mask |= bit
        if slave:
            self.port_status_mask_slave |= bit

    def set_flow_removed(self, bit, master=True, slave=True):
        if master:
            selfflow_removed_mask |= bit
        if slave:
            self.flow_removed_mask_slave |= bit
    ...
    def _pack_body(self):
        return struct.pack(
            "!IIIIII",
            self.packet_in_mask,
            self.packet_in_mask_slave,
            self.port_status_mask,
            self.port_status_mask_slave,
            self.flow_removed_mask,
            self.flow_removed_mask_slave,
        )
...
class nxt_packet_in(nicira_base, of.ofp_packet_in):
    ...
    def __eq__(self, other):
        if not of.ofp_packet_in.__eq__(self, other):
            return False
        if self.table_id != other.table_id:
            return False
        if self.cookie != other.cookie:
    ...
    *The order you add entries is significant*.  If you have an entry
    ...
    def __getattr__(self, name):
        ...
        if nxt not in self._map:
            ...
            if is_entry:
                return None  # Synthesize?
            return None

        v = self._map[nxt]
        if with_mask:
            return (v.value, v.mask)
        if is_mask:
            return v.mask
        if is_entry:
            return v
    ...
    def __setattr__(self, name, value):
        ...
        if is_entry:
            assert isinstance(value, nxm_entry)
...
def _handle_ConnectionHandshakeComplete(event):
    ...
    if not desc:
        return
    if "Open vSwitch" not in event.connection.description.hw_desc:
        return
...
def launch(convert_packet_in=False):
    ...
    core.openflow.addListenerByName(
        "ConnectionHandshakeComplete", _handle_ConnectionHandshakeComplete
    )
