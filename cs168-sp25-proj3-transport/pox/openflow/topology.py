...
class OpenFlowTopology(object):
    ...
    def _handle_openflow_discovery_LinkEvent(self, event):
        ...
        if sw1 is None or sw2 is None:
            return
        if link.port1 not in sw1.ports or link.port2 not in sw2.ports:
            return
        if event.added:
    ...
    def _handle_openflow_ConnectionDown(self, event):
        ...
        if sw is None:
            log.warn(
                "Switch %s disconnected, but... it doesn't exist!"
                % (dpidToStr(event.dpid),)
            )
        else:
            if sw._connection is None:
                log.warn(
                    "Switch %s disconnected, but... it's wasn't connected!"
                    % (dpidToStr(event.dpid),)
                )
            sw._connection = None
...
class OpenFlowSwitch(EventMixin, Switch):
    ...
    def _setConnection(self, connection, ofp=None):
        ...
        if connection is None:
            self._reconnectTimeout = Timer(
                RECONNECT_TIMEOUT, self._timer_ReconnectTimeout
            )
        if ofp is not None:
            # update capabilities
  ...
  """
    ...
    def _mod(self, entries, command):
        ...
        for entry in entries:
            if command == OFSyncFlowTable.REMOVE:
                self._pending = [
                    (cmd, pentry)
                    for cmd, pentry in self._pending
                    if not (
                        cmd == OFSyncFlowTable.ADD
                ...
                self._pending = [
                    (cmd, pentry)
                    for cmd, pentry in self._pending
                    if not (cmd == OFSyncFlowTable.ADD and entry == pentry)
                ]
    ...
    def _sync_pending(self, clear=False):
        ...
        if clear:
            ...
            self.switch.send(
                of.ofp_flow_mod(command=of.OFPFC_DELETE, match=of.ofp_match())
            )
            self.switch.send(of.ofp_barrier_request())

            todo = [
                (OFSyncFlowTable.ADD, e) for e in self.flow_table.entries
            ] + self._pending
        else:
            todo = [
                op
                ...
                or (self._pending_op_to_barrier[op][1] + OFSyncFlowTable.TIME_OUT)
                < time.time()
            ]

        for op in todo:
            fmod_xid = self.switch._xid_generator()
            flow_mod = op[1].to_flow_mod(
                xid=fmod_xid, command=op[0], flags=op[1].flags | of.OFPFF_SEND_FLOW_REM
            )
    ...
    def _handle_FlowRemoved(self, event):
        ...
        for entry in self.flow_table.entries:
            if (
                flow_removed.match == entry.match
