...
def handle_OFPST_AGGREGATE(con, parts):
    msg = parts[0].body
    e = con.ofnexus.raiseEventNoErrors(AggregateFlowStatsReceived, con, parts[0], msg)
    if e is None or e.halt != True:
    ...
    def _build_table(self):
        ...
        for fname in dir(self):
            ...
            if of_type is None:
                ...
                continue
            from_switch = getattr(
                of._message_type_to_class.get(of_type), "_from_switch", False
            )
...
class HandshakeOpenFlowHandlers(OpenFlowHandlers):
    ...
    def handle_ERROR(self, con, msg):  # A
        ...
        if msg.xid != self._barrier.xid:
            return
        if msg.type != of.OFPET_BAD_REQUEST:
            return
        if msg.code != of.OFPBRC_BAD_TYPE:
            return
    ...
    def handle_FEATURES_REPLY(self, con, msg):
        ...
        if nexus is None:
            # Cancel connection
            con.info("No OpenFlow nexus for " + pox.lib.util.dpidToStr(msg.datapath_id))
            con.disconnect()
            return
        con.ofnexus = nexus
    ...
    def handle_PORT_STATUS(self, con, msg):  # A
        if con._deferred_port_status is None:
            return
                    ...
                    try:
                        ...
                        while len(alldata):
                            ...
                            try:
                                ...
                                if e.errno != EAGAIN:
                                    con.msg(
                                        "DeferredSender/Socket error: " + e.strerror
                                    )
                                    con.disconnect()
                                    del self._dataForConnection[con]
                                break
...
class DummyOFNexus(object):
    ...
    def _disconnect(self, dpid):
        log.warning(
            "%s disconnected on dummy OpenFlow nexus", pox.lib.util.dpidToStr(dpid)
        )
...
class Connection(EventMixin):
    ...
    def send(self, data):
        ...
        try:
            ...
            if e.errno == EAGAIN:
                self.msg(
                    "Out of send buffer space.  " + "Consider increasing SO_SNDBUF."
                )
                deferredSender.send(self, data)
            else:
        ...
        while buf_len - offset >= 8:  # 8 bytes is minimum OF message size
            ...
            if self.buf[offset] != of.OFP_VERSION:
                if ofp_type == of.OFPT_HELLO:
                    ...
                    log.warning(
                        "Bad OpenFlow version (0x%02x) on connection %s"
    ...
    def _incoming_stats_reply(self, ofp):
        ...
        if not ofp.is_last_reply:
            if ofp.type not in [
                ...
                log.error(
                    "Don't know how to aggregate stats message of type " + str(ofp.type)
                )
                self._previous_stats = []
                return
        ...
        if len(self._previous_stats) != 0:
            if (ofp.xid == self._previous_stats[0].xid) and (
                ...
                log.error(
                    "Was expecting continued stats of type %i with xid %i, "
        ...
        if ofp.is_last_reply:
            ...
            if handler is None:
                log.warn(
                    "No handler for stats of type " + str(self._previous_stats[0].type)
                )
                return
            handler(self, s)
    ...
    def run(self):
        ...
        try:
            ...
            if e.errno == EADDRNOTAVAIL:
                log.error(
                    " You may be specifying a local address which is "
                    "not assigned to any interface."
                )
            elif e.errno == EADDRINUSE:
                log.error(" You may have another controller running.")
                log.error(
                    " Use openflow.of_01 --port=<port> to run POX on " "another port."
                )
            return
            ...
            try:
                        ...
                        if con is listener:
                            ...
                            if self.ssl_key or self.ssl_cert or self.ssl_ca_cert:
                                ...
                                try:
                                    ...
                                    if (
                                        exc.errno == 8
                                        and "EOF occurred" in exc.strerror
                                    ):
                ...
                if con is listener:
                    ...
                    if sock_error == ECONNRESET:
                        ...
                        log.error(
                            "Couldn't accept connection: out of file descriptors."
                        )
                    else:
                        do_close = True
                        log_tb()
