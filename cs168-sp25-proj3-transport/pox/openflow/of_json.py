...
def dict_to_match(jm):
    ...
    if "dl_vlan" in jm:
        m.dl_vlan = jm["dl_vlan"]
    if "dl_vlan_pcp" in jm:
        m.dl_vlan_pcp = jm["dl_vlan_pcp"]
...
def _init():
    ...
    good = set()
    for c in candidates:
        if c.lower() not in candidates:
            continue
        if c.upper() not in candidates:
            continue
...
def list_switches(ofnexus=None):
    ...
    for dpid, con in ofnexus._connections.items():
        ...
        for p in con.ports.values():
            pdict = {"port_no": p.port_no, "hw_addr": str(p.hw_addr), "name": p.name}
