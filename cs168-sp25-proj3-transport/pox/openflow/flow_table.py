...
class TableEntry(object):
    ...
    def show(self):
        ...
        outstr += "match=<%s>, " % (
            (self.match.show().replace("\n", " ").strip() if self.match else "Empty"),
        )
...
class FlowTable(EventMixin):
    ...
    def aggregate_stats(self, match, out_port=None):
        ...
        for entry in mc_es:
            ...
            flow_count += 1
        return ofp_aggregate_stats(
            packet_count=packet_count, byte_count=byte_count, flow_count=flow_count
        )
    ...
    def remove_matching_entries(
        self, match, priority=0, strict=False, out_port=None, reason=None
    ):
    ...
    def entry_for_packet(self, packet, in_port):
        ...
        for entry in self._table:
            if entry.match.matches_with_wildcards(
                packet_match, consider_other_wildcards=False
            ):
                return entry
