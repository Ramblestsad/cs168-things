...
def _read(data, offset, length):
    if (len(data) - offset) < length:
        raise UnderrunError(
            "wanted %s bytes but only have %s" % (length, len(data) - offset)
        )
...
def openflow_message(
    ofp_type, type_val, reply_to=None, request_for=None, switch=False, controller=False
):
...
def openflow_stats_request(stats_type, type_val=None, is_list=None, is_reply=False):
    ...
    def f(c):
        if type_val is not None:
            ...
            if ti is not None:
                ...
                ti = _stats_type_to_class_info.setdefault(type_val, _StatsClassInfo())
            _stats_class_to_type[c] = type_val
        else:
            ti = _stats_type_to_class_info.setdefault(stats_type, _StatsClassInfo())
...
class ofp_header(ofp_base):
    ...
    def pack(self):
        ...
        packed = b""
        packed += struct.pack(
            "!BBHL", self.version, self.header_type, len(self), self.xid
        )
        return packed
    ...
    def _unpack_header(self, raw, offset):
        offset, (self.version, self.header_type, length, self.xid) = _unpack(
            "!BBHL", raw, offset
        )
        return offset, length
...
class ofp_phy_port(ofp_base, ClassicCmp):
    ...
    def pack(self):
        ...
        packed += (
            self.hw_addr if isinstance(self.hw_addr, bytes) else self.hw_addr.toRaw()
        )
    ...
    def __eq__(self, other):
        ...
        if self.curr != other.curr:
            return False
        if self.advertised != other.advertised:
            return False
    ...
    def __hash__(self, *args, **kwargs):
        return (
            ...
            ^ hash(self.state)
            ^ hash(self.curr)
            ^ hash(self.advertised)
            ^ hash(self.supported) + hash(self.peer)
        )
...
class ofp_queue_prop_min_rate(ofp_base):
    ...
    def unpack(self, raw, offset=0):
        _offset = offset
        offset, (self.property, length, pad) = _unpack("!HHL", raw, offset)
...
class ofp_match(ofp_base):
    ...
    def pack(self, flow_mod=False):
        ...
        def check_tp(val):
            return (
                (val or 0)
                if self.dl_type == 0x0800 and self.nw_proto in (1, 6, 17)
                else 0
            )
        ...
        packed += struct.pack(
            "!HBB",
            self.dl_type or 0,
            check_ip(self.nw_tos),
            check_ip_or_arp(self.nw_proto),
        )
        ...
        packed += struct.pack(
            "!LLHH",
            check_ip_or_arp(fix(self.nw_src)),
            check_ip_or_arp(fix(self.nw_dst)),
            check_tp(self.tp_src),
            check_tp(self.tp_dst),
        )
    ...
    def _wire_wildcards(self, wildcards):
        ...
        if self.dl_type == 0x0800:
            ...
            return wildcards & ~(
                OFPFW_NW_SRC_MASK | OFPFW_NW_DST_MASK | OFPFW_TP_SRC | OFPFW_TP_DST
            )
        else:
    ...
    def unpack(self, raw, offset=0, flow_mod=False):
        ...
        offset, (self._dl_vlan, self._dl_vlan_pcp) = _unpack("!HB", raw, offset)
        offset = _skip(raw, offset, 1)
        offset, (self._dl_type, self._nw_tos, self._nw_proto) = _unpack(
            "!HBB", raw, offset
        )
        offset = _skip(raw, offset, 2)
    ...
    def matches_with_wildcards(self, other, consider_other_wildcards=True):
        ...
        if consider_other_wildcards:
            ...
            if (self_bits | other_bits) != self_bits:
                return False

        def match_fail(mine, others):
            if mine is None:
                return False  # Wildcarded
            return mine != others

        if match_fail(self.in_port, other.in_port):
            return False
        if match_fail(self.dl_vlan, other.dl_vlan):
            return False
        if match_fail(self.dl_src, other.dl_src):
            return False
        if match_fail(self.dl_dst, other.dl_dst):
            return False
        if match_fail(self.dl_type, other.dl_type):
            return False
        if match_fail(self.nw_proto, other.nw_proto):
            return False
        if match_fail(self.tp_src, other.tp_src):
            return False
        if match_fail(self.tp_dst, other.tp_dst):
            return False
        if match_fail(self.dl_vlan_pcp, other.dl_vlan_pcp):
            return False
        if match_fail(self.nw_tos, other.nw_tos):
            return False
        ...
        if self_nw_src[0] is not None:
            ...
            if self_nw_src[1] > other_nw_src[1]:
                return False  # ???
            if not IPAddr(other_nw_src[0]).inNetwork((self_nw_src[0], self_nw_src[1])):
                return False

        self_nw_dst = self.get_nw_dst()
        if self_nw_dst[0] is not None:
            other_nw_dst = other.get_nw_dst()
            if self_nw_dst[1] > other_nw_dst[1]:
                return False  # ???
            if not IPAddr(other_nw_dst[0]).inNetwork((self_nw_dst[0], self_nw_dst[1])):
                return False
    ...
    def __eq__(self, other):
        ...
        if self.in_port != other.in_port:
            return False
        if self.dl_src != other.dl_src:
            return False
        if self.dl_dst != other.dl_dst:
            return False
        if self.dl_vlan != other.dl_vlan:
            return False
        if self.dl_vlan_pcp != other.dl_vlan_pcp:
            return False
        if self.dl_type != other.dl_type:
            return False
        if self.nw_tos != other.nw_tos:
            return False
        if self.nw_proto != other.nw_proto:
            return False
        if self.nw_src != other.nw_src:
            return False
        if self.nw_dst != other.nw_dst:
            return False
        if self.tp_src != other.tp_src:
            return False
        if self.tp_dst != other.tp_dst:
            return False
        return True
...
class ofp_action_output(ofp_action_base):
    ...
    def unpack(self, raw, offset=0):
        _offset = offset
        offset, (self.type, length, self.port, self.max_len) = _unpack(
            "!HHHH", raw, offset
        )
        assert offset - _offset == len(self)
        return offset
...
class ofp_action_vlan_vid(ofp_action_base):
    ...
    def unpack(self, raw, offset=0):
        _offset = offset
        offset, (self.type, length, self.vlan_vid) = _unpack("!HHH", raw, offset)
        offset = _skip(raw, offset, 2)
...
class ofp_action_vlan_pcp(ofp_action_base):
    ...
    def unpack(self, raw, offset=0):
        _offset = offset
        offset, (self.type, length, self.vlan_pcp) = _unpack("!HHB", raw, offset)
    ...
    def __eq__(self, other):
        ...
        if len(self) != len(other):
            return False
        if self.vlan_pcp != other.vlan_pcp:
            return False
        return True
...
class ofp_action_dl_addr(ofp_action_base):
    ...
    def _validate(self):
        if not isinstance(self.dl_addr, EthAddr) and not isinstance(
            self.dl_addr, bytes
        ):
...
class ofp_action_tp_port(ofp_action_base):
    ...
    def unpack(self, raw, offset=0):
        _offset = offset
        offset, (self.type, length, self.tp_port) = _unpack("!HHH", raw, offset)
        offset = _skip(raw, offset, 2)
        assert offset - _offset == len(self)
        return offset
...
class ofp_features_reply(ofp_header):
    ...
    def unpack(self, raw, offset=0):
        offset, length = self._unpack_header(raw, offset)
        offset, (self.datapath_id, self.n_buffers, self.n_tables) = _unpack(
            "!QLB", raw, offset
        )
        offset = _skip(raw, offset, 3)
    ...
    def __eq__(self, other):
        if type(self) != type(other):
            return False
        if not ofp_header.__eq__(self, other):
            return False
...
class ofp_flow_mod(ofp_header):
    ...
    def unpack(self, raw, offset=0):
        ...
        offset, self.actions = _unpack_actions(
            raw, length - (32 + len(self.match)), offset
        )
        assert length == len(self)
        return offset, length
...
class ofp_port_mod(ofp_header):
    ...
    def _validate(self):
        if not isinstance(self.hw_addr, bytes) and not isinstance(
            self.hw_addr, EthAddr
        ):
    ...
    def unpack(self, raw, offset=0):
        ...
        offset, (self.port_no,) = _unpack("!H", raw, offset)
        offset, self.hw_addr = _readether(raw, offset)
        offset, (self.config, self.mask, self.advertise) = _unpack("!LLL", raw, offset)
        offset = _skip(raw, offset, 4)
        assert length == len(self)
        return offset, length
...
class ofp_stats_request(ofp_header):
    ...
    def pack(self):
        if self.type is None:
            if isinstance(self.body, ofp_stats_body_base):
                ...
                raise RuntimeError(
                    "Can't determine body type; specify it " + "explicitly"
                )
    ...
    def __eq__(self, other):
        ...
        if self.flags != other.flags:
            return False
        if self._pack_body() != other._pack_body():
            return False
        return True
...
@openflow_s_message("OFPT_STATS_REPLY", 17, reply_to="ofp_stats_request")
...
class ofp_flow_stats_request(ofp_stats_body_base):
    ...
    def unpack(self, raw, offset, avail):
        ...
        offset, (self.table_id, pad, self.out_port) = _unpack("!BBH", raw, offset)
        assert pad == 0
        assert offset - _offset == len(self)
        return offset
...
class ofp_flow_stats(ofp_stats_body_base):
    ...
    def unpack(self, raw, offset, avail):
        ...
        offset = _skip(raw, offset, 6)
        offset, (self.cookie, self.packet_count, self.byte_count) = _unpack(
            "!QQQ", raw, offset
        )
        assert (offset - _offset) == 48 + len(self.match)
        offset, self.actions = _unpack_actions(
            raw, length - (48 + len(self.match)), offset
        )
        assert offset - _offset == len(self)
        return offset
...
class ofp_aggregate_stats(ofp_stats_body_base):
    ...
    def pack(self):
        ...
        packed = b""
        packed += struct.pack(
            "!QQL", self.packet_count, self.byte_count, self.flow_count
        )
        packed += _PAD4  # Pad
        return packed
    ...
    def unpack(self, raw, offset, avail):
        _offset = offset
        offset, (self.packet_count, self.byte_count, self.flow_count) = _unpack(
            "!QQL", raw, offset
        )
        offset = _skip(raw, offset, 4)
        assert offset - _offset == len(self)
...
class ofp_table_stats(ofp_stats_body_base):
    ...
    def __eq__(self, other):
        ...
        if self.active_count != other.active_count:
            return False
        if self.lookup_count != other.lookup_count:
            return False
...
class ofp_port_stats(ofp_stats_body_base):
    ...
    def __eq__(self, other):
        ...
        if self.port_no != other.port_no:
            return False
        if self.rx_packets != other.rx_packets:
            return False
        if self.tx_packets != other.tx_packets:
            return False
        if self.rx_bytes != other.rx_bytes:
            return False
        if self.tx_bytes != other.tx_bytes:
            return False
        if self.rx_dropped != other.rx_dropped:
            return False
        if self.tx_dropped != other.tx_dropped:
            return False
        if self.rx_errors != other.rx_errors:
            return False
        if self.tx_errors != other.tx_errors:
            return False
        if self.rx_frame_err != other.rx_frame_err:
            return False
        if self.rx_over_err != other.rx_over_err:
            return False
        if self.rx_crc_err != other.rx_crc_err:
            return False
...
class ofp_queue_stats(ofp_stats_body_base):
    ...
    def pack(self):
        ...
        packed += _PAD2
        packed += struct.pack(
            "!LQQQ", self.queue_id, self.tx_bytes, self.tx_packets, self.tx_errors
        )
        return packed
...
class ofp_packet_out(ofp_header):
    ...
    def unpack(self, raw, offset=0):
        _offset = offset
        offset, length = self._unpack_header(raw, offset)
        offset, (self._buffer_id, self.in_port, actions_len) = _unpack(
            "!LHH", raw, offset
        )
    ...
    def __len__(self):
        return (
            16
            + reduce(operator.add, (len(a) for a in self.actions), 0)
    ...
    def show(self, prefix=""):
        ...
        for obj in self.actions:
            if obj is None:
                raise RuntimeError(
                    "An element of self.actions was None! " + "Bad formatting..."
                )
            outstr += obj.show(prefix + "  ")
        return outstr
...
##3.7 Barrier Message
@openflow_s_message("OFPT_BARRIER_REPLY", 19, reply_to="ofp_barrier_request")
...
@openflow_c_message("OFPT_BARRIER_REQUEST", 18, request_for="ofp_barrier_reply")
...
class ofp_packet_in(ofp_header):
    ...
    def pack(self):
        ...
        packed += ofp_header.pack(self)
        packed += struct.pack(
            "!LHHBB", self._buffer_id, self.total_len, self.in_port, self.reason, 0
        )
        packed += self.data
    ...
    def unpack(self, raw, offset=0):
        offset, length = self._unpack_header(raw, offset)
        offset, (self._buffer_id, self._total_len, self.in_port, self.reason, pad) = (
            _unpack("!LHHBB", raw, offset)
        )
...
class ofp_flow_removed(ofp_header):
    ...
    def pack(self):
        ...
        packed += _PAD
        packed += struct.pack(
            "!LLH", self.duration_sec, self.duration_nsec, self.idle_timeout
        )
        packed += _PAD2
    ...
    def unpack(self, raw, offset=0):
        offset, length = self._unpack_header(raw, offset)
        offset = self.match.unpack(raw, offset)
        offset, (self.cookie, self.priority, self.reason) = _unpack("!QHB", raw, offset)
        offset = _skip(raw, offset, 1)
        offset, (self.duration_sec, self.duration_nsec, self.idle_timeout) = _unpack(
            "!LLH", raw, offset
        )
        offset = _skip(raw, offset, 2)
        offset, (self.packet_count, self.byte_count) = _unpack("!QQ", raw, offset)
        assert length == len(self)
        return offset, length
...
@openflow_sc_message("OFPT_ECHO_REQUEST", 2, request_for="ofp_echo_reply")
...
@openflow_sc_message("OFPT_ECHO_REPLY", 3, reply_to="ofp_echo_request")
...
@openflow_c_message("OFPT_FEATURES_REQUEST", 5, request_for="ofp_features_reply")
...
@openflow_c_message("OFPT_GET_CONFIG_REQUEST", 7, request_for="ofp_get_config_reply")
...
@openflow_s_message("OFPT_GET_CONFIG_REPLY", 8, reply_to="ofp_get_config_request")
...
def _init():
    def formatMap(name, m):
        ...
        maxlen = 2 + len(
            reduce(lambda a, b: a if len(a) > len(b) else b, (v for k, v in vk))
        )
        fstr = "  %-" + str(maxlen) + "s : %s,\n"
        for v, k in vk:
            o += fstr % ("'" + k + "'", v)
