...
def is_down(p):
    if p.config & of.OFPPC_PORT_DOWN:
        return True
    if p.state & of.OFPPS_LINK_DOWN:
        return True
    return False
...
class Switch(object):
    ...
    def _compute(self):
        ...
        for p in con.ports.values():
            ...
            if not fld:
                out[p.no] |= of.OFPPC_NO_FLOOD
            if not rcv:
                out[p.no] |= of.OFPPC_NO_FWD
    ...
    def _realize(self):
        if self._port_out == self._port_out_cache:
            return
...
class SpanningForest(object):
    ...
    def _compute_nx(self):
        ...
        self.log.debug(
            "Computed spanning forest: %s of %s links",
            tree.size(),
            len(self.topo.links),
        )
    ...
    def _compute_simple(self, stable=True, randomize=False):
        ...
        def add_links(links):
            for bl in links:
                l = bl.link
                if l.dpid2 in reachable.setdefault(
                    l.dpid1, set([l.dpid1])
                ) or l.dpid1 in reachable.setdefault(l.dpid2, set([l.dpid2])):
                    # Already reachable
                    continue
                for o in reachable.get(l.dpid2, set([l.dpid2])):
                    reachable[l.dpid1].add(o)
                    if o in reachable:
                        reachable[o] = reachable[l.dpid1]
                used.append(bl)
        ...
        if stable:
            ...
            self.log.debug(
                "Computing spanning forest.  New links:%s Reused:%s",
                len(links),
                len(prev),
            )
            add_links(prev)
        ...
        self.log.debug(
            "Spanning forest computed.  Components:%s  Links:%s",
            len(set(id(x) for x in reachable.values())),
            len(used),
        )
