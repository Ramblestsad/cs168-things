...
class POXCore(EventMixin):
    ...
    def __init__(
        self, threaded_selecthub=True, epoll_selecthub=False, handle_signals=True
    ):
        self.debug = False
        self.up = False
        self.running = True
    ...
    def goUp(self):
        ...
        if vers not in good_versions:
            vwarn(
                "POX requires one of the following versions of Python: %s",
                " ".join(good_versions),
            )
    ...
    def _goUp_stage2(self, main=False):
        with self._deferral_lock:
            if main:
                self._ready_to_go_up = True
            if not self._ready_to_go_up:
                return
    ...
    def _waiter_notify(self):
        if len(self._waiters):
            ...
            for entry in self._waiters:
                ...
                log.debug("%s still waiting for: %s" % (name, " ".join(components)))
        ...
        if name is None:
            ...
            if name is None:
                ...
                if hasattr(callback, "__self__"):
                    name = (
                        getattr(callback.__self__.__class__, "__name__", "")
                        + "."
                        + name
                    )
    ...
    def listen_to_dependencies(
        self, sink, components=None, attrs=True, short_attrs=False, listen_args={}
    ):
        """
        ...
        *NOTE*: The semantics of this function changed somewhat in the
        ...
        if set(listen_args).difference(components):
            log.error(
                "Specified listen_args for missing component(s): %s"
...
def initialize(threaded_selecthub=True, epoll_selecthub=False, handle_signals=True):
    global core
    core = POXCore(
        threaded_selecthub=threaded_selecthub,
