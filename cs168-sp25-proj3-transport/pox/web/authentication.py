...
class BasicAuthMixin(object):
    ...
    def _check_basic_auth(self, user, password):
        ...
        """
        if self._is_basic_auth_enabled is False:
            return True
    ...
    def _is_basic_auth_enabled(self):
        ...
        try:
            if (
                self._check_basic_auth.__func__.__code__
                is BasicAuthMixin._check_basic_auth.__func__.__code__
            ):
    ...
    def _get_auth_realm(self):
        ...
        if auth_realm is True:
            ...
            if r.endswith("Handler"):
                r = r.rsplit("Handler", 1)[0]
            return r
        else:
            return auth_realm
    ...
    def _send_basic_auth_header(self):
        self.send_header(
            "WWW-Authenticate", 'Basic realm="%s"' % (self._get_auth_realm(),)
        )
