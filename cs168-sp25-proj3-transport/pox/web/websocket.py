...
class WebsocketHandler(SplitRequestHandler, object):
    ...
    def _serve_websocket(self):
        ...
        if do_cg is None:
            do_cg = getattr(self.parent, "pox_cookieguard", True)
        if not do_cg:
            cg_ok = True
        else:
    ...
    def do_GET(self):
        ...
        if hasattr(self, "_do_auth") and not self._do_auth():
            return
        ...
        if self._lock:
            with self._lock:
                if self._pending:
                    ...
                    self._pending = True
        if cl:
            core.call_later(self._ws_message2)
...
def log_service():
    ...
    def ready():
        ...
        if addr[0] == "0.0.0.0":
            addr[0] = "127.0.0.1"
        docs = {
            "/": _log_page.replace("SERVER_ADDRESS", "%s:%s" % tuple(addr)),
