...
class JSONRPCHandler(SplitRequestHandler):
    ...
    def _send_auth_header(self):
        if self.auth_function:
            self.send_header(
                "WWW-Authenticate", 'Basic realm="%s"' % (self.auth_realm,)
            )
    ...
    def do_POST(self):
        ...
        try:
            ...
            response = {}
            response["error"] = {"code": self.ERR_PARSE_ERROR, "message": "Parse error"}
...
def make_error(msg="Unknown Error", code=JSONRPCHandler.ERR_SERVER_ERROR, data=None):
