        ...
        for s, (r, w, c) in cc.items():
            ...
            if c:
                try:
                    ...
                    pass
        if cc:
            log.debug("Shut down %s socket(s)", len(cc))
...
class POXCookieGuardMixin(object):
    ...
    def _cookieguard_maybe_consume_post(self):
        if self._pox_cookieguard_consume_post is False:
            return
    ...
    def _do_cookieguard_explict_continuation(self, requested, target):
        ...
        """
        log.debug(
            "POX CookieGuard bouncer doesn't have correct cookie; "
    ...
    def _do_cookieguard_set_cookie(self, requested, bad_cookie):
        ...
        self.send_header(
            "Location", self._pox_cookieguard_bouncer + "?" + quote_plus(requested)
        )
        self.end_headers()
    ...
    def _do_cookieguard(self, override=None):
        do_cg = override
        if do_cg is None:
            do_cg = getattr(self, "pox_cookieguard", True)
        if not do_cg:
            return True
...
class SplitRequestHandler(BaseHTTPRequestHandler):
    ...
    def set_permissive_cors(self):
        ...
        if self.ac_allow_methods is None:
            self.ac_allow_methods = set()
        self.ac_allow_methods.update(["GET", "OPTIONS", "POST"])
        if self.ac_allow_headers is None:
            self.ac_allow_headers = set()
    ...
    def log_request(self, code="-", size="-"):
        weblog.debug(
            self.prefix + (':"%s" %s %s' % (self.requestline, str(code), str(size)))
        )
...
_favicon = (
    "47494638396110001000c206006a5797927bc18f83ada9a1bfb49ceabda"
...
_favicon = bytes(int(_favicon[n : n + 2], 16) for n in range(0, len(_favicon), 2))
...
class CoreHandler(SplitRequestHandler):
    ...
    def send_info(self, is_get=False):
        ...
        r += "<ul>"
        m = [
            list(map(cgi_escape, map(str, [x[0], x[1], x[1].format_info(x[3])])))
...
class StaticContentHandler(SplitRequestHandler, SimpleHTTPRequestHandler):
    ...
    def send_head(self):
        ...
        if os.path.isdir(path):
            ...
            and self.zip_directories
            and self.path.endswith("/.zip")
        ):
    ...
    def translate_path(self, path, include_prefix=True):
        ...
        def fixpath(p):
            ...
            while True:
                ...
                if p in ("/", "") and tail == "":
                    break
                if tail in ("", ".", os.path.curdir, os.path.pardir):
                    continue
                if os.path.sep in tail:
                    continue
                if os.path.altsep and os.path.altsep in tail:
                    continue
                if os.path.splitdrive(tail)[0] != "":
                    continue
...
def wrapRequestHandler(handlerClass):
    return type(
        "Split" + handlerClass.__name__, (SplitRequestHandler, handlerClass, object), {}
    )
...
class SplitCGIRequestHandler(SplitRequestHandler, CGIHTTPRequestHandler, object):
    """
...
class SplitterRequestHandler(
    BaseHTTPRequestHandler, BasicAuthMixin, POXCookieGuardMixin
):
    ...
    def log_request(self, code="-", size="-"):
        weblog.debug('splitter:"%s" %s %s', self.requestline, str(code), str(size))
    ...
    def _check_basic_auth(self, user, password):
        if self.basic_auth_info.get(user) == password:
            return True
    ...
    def handle_one_request(self):
        ...
        if not self._do_cookieguard(override_cg):
            return
...
class SplitThreadedServer(ThreadingMixIn, HTTPServer, EventMixin):
    ...
    def __init__(self, *args, **kw):
        ...
        if self.ssl_server_key or self.ssl_server_cert or self.ssl_client_certs:
            ...
            self.socket = ssl.wrap_socket(
                self.socket,
                server_side=True,
                keyfile=self.ssl_server_key,
                certfile=self.ssl_server_cert,
                ca_certs=self.ssl_client_certs,
                cert_reqs=cert_reqs,
    ...
    def add_static_dir(self, www_path, local_path=None, relative=False):
        ...
        self.set_handler(www_path, StaticContentHandler, {"root": local_path}, True)
