...
class SwitchConnectionDown(SwitchEvent):
    pass
...
class Topology(EventMixin):
    ...
    def addListener(
        self, eventType, handler, once=False, weak=False, priority=None, byName=False
    ):
        """
    ...
    def deserializeAndMerge(self, id2entity):
        ...
        for entity_id in id2entity.keys():
            ...
            if existing_entity:
                self.log.debug(
                    "New metadata for %s: %s " % (str(existing_entity), str(entity))
                )
