...
class HostEvent(Event):
    ...
    def __init__(
        self, entry, new_dpid=None, new_port=None, join=False, leave=False, move=False
    ):
...
class host_tracker(EventMixin):
    ...
    def __init__(self, ping_src_mac=None, install_flow=True, eat_packets=True):
        ...
        self.entryByMAC = {}
        self._t = Timer(
            timeoutSec["timerInterval"], self._check_timeouts, recurring=True
        )
    ...
    def sendPing(self, macEntry, ipAddr):
        ...
        if core.openflow.sendToDPID(macEntry.dpid, msg.pack()):
            ...
            log.debug(
                "%i %i ERROR sending ARP REQ to %s %s",
                macEntry.dpid,
                macEntry.port,
                str(r.hwdst),
                str(r.protodst),
    ...
    def getSrcIPandARP(self, packet):
        ...
        if isinstance(packet, ipv4):
            ...
            log.debug(
                "ARP %s %s => %s",
                {arp.REQUEST: "request", arp.REPLY: "reply"}.get(
                    packet.opcode, "op:%i" % (packet.opcode,)
                ),
                str(packet.protosrc),
                str(packet.protodst),
            )
            if (
                packet.hwtype == arp.HW_TYPE_ETHERNET
                and packet.prototype == arp.PROTO_TYPE_IP
                and packet.protosrc != 0
            ):
    ...
    def updateIPInfo(self, pckt_srcip, macEntry, hasARP):
        ...
        if pckt_srcip in macEntry.ipAddrs:
            ...
            ipEntry.refresh()
            log.debug(
                "%s already has IP %s, refreshing", str(macEntry), str(pckt_srcip)
            )
        else:
            # new mapping
    ...
    def _handle_openflow_PacketIn(self, event):
        ...
        if macEntry is None:
            ...
            if time.time() - macEntry.lastTimeSeen < timeoutSec["entryMove"]:
                log.warning(
                    "Possible duplicate: %s at time %i, now (%i %i), time %i",
                    str(macEntry),
                    macEntry.lastTimeSeen,
                    dpid,
                    inport,
                    time.time(),
                )
            ...
            if macEntry.expired() and not entryPinged:
                ...
                if len(macEntry.ipAddrs) > 0:
                    for ip_addr in macEntry.ipAddrs.keys():
                        log.warning(
                            "Entry %s expired but still had IP address %s",
                            str(macEntry),
                            str(ip_addr),
                        )
