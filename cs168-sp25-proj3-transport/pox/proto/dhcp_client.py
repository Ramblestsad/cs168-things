...
class DHCPClientBase(EventMixin):
  ...
  * Handle events
  """
    ...
    _eventMixin_events = set([DHCPOffer, DHCPOffers, DHCPLeased, DHCPClientError])
    ...
    def state(self, state):
        ...
        if state == self.INIT:
            ...
            if old == self.NEW:
                ...
                killtimer("total")
                self.total_timer = recoco.Timer(
                    self.total_timeout, self._do_total_timeout
                )
                self._start = time.time()
            self._discover()
            self.discover_timer = recoco.Timer(
                self.discover_timeout, set_state(self.INIT)
            )
        elif state == self.SELECTING:
            assert old == self.INIT
            self.offer_timer = recoco.Timer(self.offer_timeout, self._do_accept)
            ...
            if not routers:
                routers = "(No routers)"
            self.log.info(
                "Got %s/%s -> %s", self.bound.address, self.bound.subnet_mask, routers
            )
  ...
  """
    ...
    def _listen_for_connection(self):
        core.openflow.addListenerByName(
            "ConnectionUp", self._handle_ConnectionUp, once=True
        )
