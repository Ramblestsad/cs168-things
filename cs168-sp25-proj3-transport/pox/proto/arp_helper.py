...
def send_arp_request(connection, ip, port=of.OFPP_FLOOD, src_mac=False, src_ip=None):
    """
    Send an ARP request
        ...
        if port in (of.OFPP_FLOOD, of.OFPP_ALL):
            for p in list(connection.ports.values()):
                ...
                if p.port_no > of.OFPP_MAX:
                    continue  # Off by one?
                send_arp_request(
                    connection, ip, p.port_no, src_mac=p.hw_addr, src_ip=src_ip
                )
            return
...
class ARPReply(Event):
    ...
    def __str__(self):
        return "ARPReply for %s on %s" % (self.reply.protodst, dpid_to_str(self.dpid))
...
class ARPHelper(EventMixin):
    ...
    def send_arp_request(
        self, connection, ip, port=of.OFPP_FLOOD, src_mac=_default_mac, src_ip=None
    ):
    ...
    def _handle_PacketIn(self, event):
        ...
        if a.opcode == arp.REQUEST:
            log.debug(
                "%s ARP request %s => %s", dpid_to_str(dpid), a.protosrc, a.protodst
            )

            src_mac = _default_mac
            ev = ARPRequest(event.connection, a, src_mac, self.eat_packets, inport)
            self.raiseEvent(ev)
            if ev.reply is not None:
                log.debug(
                    "%s answering ARP for %s" % (dpid_to_str(dpid), str(a.protodst))
                )
