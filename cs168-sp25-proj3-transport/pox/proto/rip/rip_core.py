...
class Entry(object):
    ...
    def __init__(
        self, owner, data=None, origin=None, dev=None, static=False, local=False
    ):
        self.changed = True
        self.t = None  # Timer
...
class RIPRouter(object):
    ...
    def get_responses(self, dests, force, static_only=False, mtu=DEFAULT_MTU):
        ...
        for e in self.table.values():
            if not (e.changed or force):
                continue
            if static_only and not e.static:
                continue
            re = RIP.RIPEntry()
    ...
    def process_response(self, iface, addr, ripp):
        ...
        for e in ripp.entries:
            if e.address_family != socket.AF_INET:
                continue
            if e.route_tag != 0:
