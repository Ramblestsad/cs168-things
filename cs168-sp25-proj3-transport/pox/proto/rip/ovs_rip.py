...
class OVSRIPRouter(RIPRouter):
    ...
    def _deferred_sync_table(self):
        ...
        if self._deferred_sync_table_pending > 1:
            return

        def do_it():
            self.log.debug(
                "Syncing table after %s deferrals", self._deferred_sync_table_pending
            )
            self._deferred_sync_table_pending = 0
            self.sync_table()
    ...
    def _refresh_ports(self):
        ...
        for name, ip_prefix_pairs in self._port_cache.items():
            if name not in self._conn.ports:
                continue
    ...
    def _init_ingress_table(self):
        ...
        for portno, portobj in self._ports.items():
            if portno not in self._conn.ports:
                continue
        ...
        fm.priority -= 1
        fm.match.dl_type = pkt.ethernet.IP_TYPE
        fm.actions.append(ovs.nx_reg_move(dst=DST_IP_REGISTER, src=ovs.NXM_OF_IP_DST))
    ...
    def _do_arp_table(self, event):
        ...
        arp_sent, entry = port.arp_table.send(
            event.parsed,
            router_ip=router_ip,
        ...
        if arp_sent:
            self.log.debug(
                "ARPed for %s", router_ip if router_ip is not None else real_dst_ip
            )
        else:
            self.log.debug(
                "Used controller ARP entry for %s",
    ...
    def _add_arp_entry(self, ip_or_arp, eth=None):
        ...
        fm.actions.append(ovs.nx_reg_move(src=DST_IP_REGISTER, dst=ovs.NXM_OF_IP_DST))
    ...
    def sync_table(self):
        ...
        for e in self.table.values():
            ...
            if e.dev is not None:
                ...
                if port is None:
                    continue
                fm.actions.append(ovs.nx_reg_load(dst=OUT_PORT_REGISTER, value=e.dev))
        ...
        if self._conn:
            ...
            self.log.debug(
                "Syncing %s port and %s net table entries",
