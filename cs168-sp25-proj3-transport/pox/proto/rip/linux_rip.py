...
def _cmd(e):
    ...
    if e.dev:
        return "%s dev %s metric %s" % (ips, e.dev, e.metric)
    else:
        return "%s via %s metric %s" % (ips, e.next_hop, e.metric)
...
class LinuxRIPRouter(RIPRouter, Task):
    ...
    def add_local_routes(self, table=None):
        ...
        for e in d:
            ...
            n.metric = 1
            if "metric" in e:
                n.metric = int(e[e.index("metric") + 1])
            self.table[n.key] = n
    ...
    def sync_table(self):
        ...
        for e in remove:
            ...
            if e not in modify:
                ...
                self.log.info(
                    "%s route for %s", "Modifying" if e in modify else "Removing", e
                )
            runfail(*cmd.split())
        for e in add:
            cmd = "ip route add " + _cmd(e) + " table " + str(self.tableno)
            self.log.info(
                "%s route for %s", "Modifying" if e.key in modify else "Adding", e.key
            )
            runfail(*cmd.split())
