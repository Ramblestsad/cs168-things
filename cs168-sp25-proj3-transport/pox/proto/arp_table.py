...
class ARPTable(object):
    ...
    def send(
        self, eth_packet, router_ip=None, src_eth=None, src_ip=None, send_function=None
    ):
        """
        ...
        if dstip not in self.by_ip:
            self.add_entry(dstip)
        ...
        if e.mac is not None:
            ...
            if len(self.pending) < self.MAX_PENDING:
                self.pending.append(
                    (dstip, (eth_packet, router_ip, src_eth, src_ip, send_function))
                )
