...
class NAT(object):
    def __init__(
        self, inside_ip, outside_ip, gateway_ip, dns_ip, outside_port, dpid, subnet=None
    ):
    ...
    def _all_dependencies_met(self):
        ...
        if self.dpid in core.openflow.connections:
            ...
            core.openflow.addListenerByName(
                "ConnectionUp", self.__handle_dpid_ConnectionUp
            )
    ...
    def _is_local(self, ip):
        ...
        if self.subnet is not None:
            if ip.in_network(self.subnet):
                return True
            return False
        if ip.in_network("192.168.0.0/16"):
            return True
        if ip.in_network("10.0.0.0/8"):
            return True
        if ip.in_network("172.16.0.0/12"):
            return True
        return False
    ...
    def _arp_for_gateway(self):
        ...
        self._ARPHelper_.send_arp_request(
            self._connection,
            ip=self.gateway_ip,
...
class NATDHCPD(DHCPD):
    ...
    def _handle_PacketIn(self, event):
        if self._dpid != event.dpid:
            return
        if event.port == self._outside_port_no:
            return
...
def launch(dpid, outside_port, subnet="172.16.1.0/24", inside_ip="172.16.1.1"):
    ...
    def got_lease(event):
        ...
        n = NAT(
            inside_ip, outside_ip, gateway_ip, dns_ip, outside_port, dpid, subnet=subnet
        )
        core.register(n)
