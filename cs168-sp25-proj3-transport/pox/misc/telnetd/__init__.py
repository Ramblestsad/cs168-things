...
class OptTelnetHandler(QTelnetHandler):
    ...
    def _init_wants(
        self, want_to=None, want_to_not=None, want_for=None, want_for_not=None
    ):
        def s(v):
            if not v:
                return set()
...
class LineEdit(object):
    ...
    def _do_char(self, c):
        if c == b"\x7f" or c == b"\x08":
            ...
            self.__current = (
                self.__current[: self.__cursor] + c + self.__current[self.__cursor :]
            )
            self.__cursor += 1
...
class TelnetWorker(RecocoIOWorker, MyTelnetHandler, LineEdit):
    ...
    def __init__(self, personality_class, personality_kwargs, *args, **kw):
...
class TelnetPersonality(object):
    ...
    def send(self, msg):
        ...
        Send to client
        """
        if self.encoding and isinstance(msg, str):
...
class PythonTelnetPersonality(TelnetPersonality):
    ...
    def _handle_line(self, line):
        ...
        try:
            ...
            if gettrace():
                raise RuntimeError()  # Someone already tracing
        except:
...
def launch(username="pox", password=None, port=2323):
    ...
    core.registerNew(
        TelnetServer,
        personality=PythonTelnetPersonality,
        personality_kwargs=kw,
        port=int(port),
    )
