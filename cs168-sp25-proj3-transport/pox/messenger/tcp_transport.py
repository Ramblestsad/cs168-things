        ...
        while core.running:
            ...
            while core.running:
                ...
                if delay > self._max_backoff:
                    ...
                    if show_notices:
                        self.log.debug(
                            "Connection to %s:%s failed.  Retrying in %s " "seconds.",
                            self._addr[0],
                            self._addr[1],
                            delay,
                        )
                        self.log.debug(
                            "(Further reconnect messages will be squelched.)"
                        )
                    show_notices = False
                elif show_notices:
                    self.log.debug(
...
class TCPTransport(Task, Transport):
    def __init__(
        self, address="0.0.0.0", port=7790, nexus=None, connection_class=TCPConnection
    ):
        port = int(port)
        Task.__init__(self)
        Transport.__init__(self, nexus)
