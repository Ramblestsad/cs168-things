...
class DefaultChannelBot(ChannelBot):
    ...
    def _exec_cmd_invite(self, event):
        ...
        if chan is None:
            # TODO: send an error
            log.warning("A bot was invited to a nonexistent channel (%s)" % (channel,))
            return
        if botclass is None:
            # TODO: send an error
            log.warning("A nonexistent bot (%s) was invited to a channel" % (botname,))
            return
    ...
    def _unhandled(self, event):
        log.warn("Default channel got unknown command: " + str(event.msg.get("cmd")))
