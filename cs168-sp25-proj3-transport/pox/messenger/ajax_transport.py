    ...
    def tx(self, wfile, seq, batch_size):
        ...
        if seq is None:
            ...
            if seq > self._sent_tx_seq + 1:
                ...
                log.debug(
                    "Client is living in the future (they sent seq %s but "
                    + "we are only at %s)",
                    seq,
                    self._sent_tx_seq + 1,
                )
        ...
        with self._cond:
            ...
            if self._quitting:
                ...
                ##self._cond.release()
                return _result(
                    {"messages": [], "failure": "quit", "seq": self._sent_tx_seq}
                )
            ...
            if len(data) > 0:
                if seq < data[0][0]:
                    ...
                    return _result(
                        {"messages": [], "failure": "expired", "seq": self._sent_tx_seq}
                    )
...
def launch(username="", password=""):
    ...
    core.call_when_ready(
        _launch, ["WebServer", "MessengerNexus"], name="ajax_transport"
    )
