    ...
    for name in component_order:
        ...
        if launch in members:
            ...
            if f.__code__.co_argcount > 0:
                ...
                if f.__code__.co_varnames[f.__code__.co_argcount - 1] == "__INSTANCE__":
            ...
            try:
                ...
                if inspect.currentframe() is sys.exc_info()[2].tb_frame:
                    ...
                    if len(args) == 0:
                        ...
                        print(
                            " {0:25} {1:25} {2:25}".format("Name", "Default", "Active")
                        )
                        print(" {0:25} {0:25} {0:25}".format("-" * 15))

                        for k, v in args.items():
                            print(
                                " {0:25} {1:25} {2:25}".format(
                                    k,
                                    str(v[0]),
                    ...
                    if len(params):
                        ...
                        return False
                    missing = [
                        k for k, x in args.items() if x[1] is EMPTY and x[0] is EMPTY
                    ]
                    if len(missing):
                        print(
                            "You must specify a value for the '{0}' "
            ...
            print(
                "Module %s has no %s(), but it was specified or passed "
...
def set_main_function(f):
    ...
    if _main_thread_function is not None:
        ...
        lg.error(
            "The main thread function is already "
            + "taken by: "
            + str(_main_thread_function)
        )
        return False
