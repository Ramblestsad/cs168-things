...
parser.add_option(
    "-f",
    "--force",
    help="force overwriting existing unit "
    ...
    if not os.path.exists(test_file) or options.force:
        ...
        if match:
            ...
            if read_sha1 == calculated_sha1:
                ...
                print(
                    (
                        "Test for %s in %s already exists (and sha1 sums don't "
                        "match: %s<=>%s)"
                    )
                    % (test_class_name, test_file, read_sha1, calculated_sha1)
                )
        else:
            print(
                (
                    "Test for %s in %s already exists (and no autogeneration "
                    "sig found)"
                )
