...
class SwitchImplTest(unittest.TestCase):
    ...
    def test_echo_request(self):
        ...
        self.assertEqual(len(c.received), 1)
        self.assertTrue(
            isinstance(c.last, ofp_echo_reply) and c.last.xid == 123,
    ...
    def test_barrier(self):
        ...
        self.assertEqual(len(c.received), 1)
        self.assertTrue(
            isinstance(c.last, ofp_barrier_reply) and c.last.xid == 123,
            "should have received echo reply but got %s" % c.last,
        )
    ...
    def test_flow_mod(self):
        ...
        c.to_switch(
            ofp_flow_mod(
                xid=124, priority=1, match=ofp_match(in_port=1, nw_src="1.2.3.4")
            )
        )
    ...
    def test_send_packet_in(self):
        ...
        s = self.switch
        s.send_packet_in(
            in_port=1, buffer_id=123, packet=self.packet, reason=OFPR_NO_MATCH
        )
        self.assertEqual(len(c.received), 1)
        self.assertTrue(
            isinstance(c.last, ofp_packet_in) and c.last.xid == 0,
    ...
    def test_rx_packet(self):
        ...
        self.assertEqual(len(c.received), 1)
        self.assertTrue(
            isinstance(c.last, ofp_packet_in),
            "should have received packet_in but got %s" % c.last,
        )
    ...
    def test_delete_port(self):
        ...
        self.assertTrue(
            new_num_ports == original_num_ports - 1, "Should have removed the port"
        )
        self.assertEqual(len(c.received), 1)
        self.assertTrue(
            isinstance(c.last, ofp_port_status),
    ...
    def test_add_port(self):
        ...
        self.assertTrue(
            port_count - 1 == len(self.switch.ports), "Should have removed port"
        )
        self.assertFalse(
            old_port.port_no in self.switch.ports, "Should have removedport"
        )
        s.add_port(old_port)
        self.assertTrue(old_port.port_no in self.switch.ports, "Should have added port")
...
class ProcessFlowModTest(unittest.TestCase):
    ...
    def test_process_flow_mod_add(self):
        ...
        # test wrong port
        msg = ofp_flow_mod(
            priority=5, cookie=0x31415926, actions=[ofp_action_output(port=5)]
        )
        c.to_switch(msg)
    ...
    def test_process_flow_mod_modify(self):
        ...
        def table():
            ...
            t.add_entry(
                TableEntry(
                    ...
                    cookie=0x1,
                    match=ofp_match(
                        dl_src=EthAddr("00:00:00:00:00:01"), nw_src="1.2.3.4"
                    ),
            ...
            t.add_entry(
                TableEntry(
                    ...
                    cookie=0x2,
                    match=ofp_match(
                        dl_src=EthAddr("00:00:00:00:00:02"), nw_src="1.2.3.0/24"
                    ),
                    actions=[ofp_action_output(port=6)],
                )
            )
            t.add_entry(
                TableEntry(priority=1, cookie=0x3, match=ofp_match(), actions=[])
            )
            return t
        ...
        t = s.table
        msg = ofp_flow_mod(
            command=OFPFC_MODIFY, match=ofp_match(), actions=[ofp_action_output(port=1)]
        )
        c.to_switch(msg)
        self.assertEquals(
            [e.cookie for e in t.entries if e.actions == [ofp_action_output(port=1)]],
            [1, 2, 3],
        )
        ...
        msg = ofp_flow_mod(
            ...
            match=ofp_match(nw_src="1.2.0.0/16"),
            actions=[ofp_action_output(port=8)],
        )
        c.to_switch(msg)
        self.assertEquals(
            [e.cookie for e in t.entries if e.actions == [ofp_action_output(port=8)]],
            [1, 2],
        )
        self.assertEquals(len(t.entries), 3)
        ...
        msg = ofp_flow_mod(
            cookie=5,
            command=OFPFC_MODIFY,
            match=ofp_match(nw_src="2.2.0.0/16"),
            actions=[ofp_action_output(port=8)],
        )
        c.to_switch(msg)
    ...
    def test_process_flow_mod_modify_strict(self):
        ...
        msg = ofp_flow_mod(
            command=OFPFC_MODIFY_STRICT,
            priority=5,
            match=ofp_match(dl_src=EthAddr("00:00:00:00:00:02"), nw_src="1.2.3.0/24"),
            actions=[ofp_action_output(port=8)],
        )
        c.to_switch(msg)
        self.assertEquals(
