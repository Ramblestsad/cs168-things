...
class OFSyncFlowTableTest(unittest.TestCase):
    ...
    def test_reconnect_pending(self):
        ...
        self.assertTrue(
            isinstance(s.sent[-4], ofp_flow_mod)
            and s.sent[-4].command == OFPFC_DELETE
            and s.sent[-4].match == ofp_match()
        )
        self.assertTrue(isinstance(s.sent[-3], ofp_barrier_request))
        self.assertTrue(
            isinstance(s.sent[-2], ofp_flow_mod)
            and s.sent[-2].command == OFPFC_ADD
            and s.sent[-2].match == entry.match
        )
        self.assertTrue(isinstance(s.sent[-1], ofp_barrier_request))
    ...
    def test_install_remove(self):
        ...
        self.assertTrue(
            isinstance(seen_ft_events[0], FlowTableModification)
            and seen_ft_events[-1].added == [entry]
        )
        ...
        self.assertTrue(
            isinstance(s.sent[-2], ofp_flow_mod)
            and s.sent[-2].command == OFPFC_DELETE_STRICT
        )
        self.assertTrue(isinstance(s.sent[-1], ofp_barrier_request))
        ...
        self.assertTrue(
            isinstance(seen_ft_events[-1], FlowTableModification)
            and seen_ft_events[-1].removed == [entry]
        )

    def test_handle_FlowRemoved(self):
        """test that simple removal of a flow works"""
        t = self.t
        t.flow_table.add_entry(
            TableEntry(
                priority=5,
                cookie=0x31415926,
                match=ofp_match(dl_src=EthAddr("00:00:00:00:00:01")),
        ...
        for non_matching in [
            ...
            },  # cookie doesn't fit
            {
                "cookie": 0x31415927,
                "match": ofp_match(
                    dl_src=EthAddr("00:00:00:00:00:02"), nw_src=IPAddr("1.2.3.4")
                ),
            },  # extra match field
        ]:
            t._handle_FlowRemoved(
                FlowRemoved(self.conn, ofp_flow_removed(**non_matching))
            )
            self.assertEqual(len(t), 1)
            self.assertEqual(t.entries[0].cookie, 0x31415927)
