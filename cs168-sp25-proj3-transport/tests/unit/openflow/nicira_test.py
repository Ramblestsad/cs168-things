...
class basics_test(unittest.TestCase):
    ...
    def test_unpack_weird_header(self):
        ...
        self.assertEqual(offset, len(original_packed), "Didn't unpack entire entry")
        unoriginal_packed = unoriginal.pack()

        self.assertEqual(
            unoriginal.dst.__name__,
            "NXM_UNKNOWN_dead_42",
    ...
    def test_action_pack_unpack(self):
        ...
        for name in dir(nx):
            a = getattr(nx, name)
            if not nx._issubclass(a, of.ofp_action_vendor_base):
                continue
            ...
            offset = unoriginal.unpack(original_packed, 0)
            self.assertEqual(
                offset, len(original_packed), "Didn't unpack entire entry " + name
            )
            unoriginal_packed = unoriginal.pack()

            self.assertEqual(original, unoriginal, "Pack/Unpack failed for " + name)
            print("Success!")
    ...
    def test_nxm_ip(self):
        ...
        self.assertRaisesRegexp(AssertionError, "^nonzero masked bits$", try_bad)
    ...
    def test_match_pack_unpack(self):
        ...
        for nxm_name, nxm_type in nx._nxm_name_to_type.items():
            ...
            self.assertEqual(
                offset, len(original_packed), "Didn't unpack entire entry " + nxm_name
            )
            unoriginal_packed = unoriginal.pack()

            self.assertEqual(original, unoriginal, "Pack/Unpack failed for " + nxm_name)
