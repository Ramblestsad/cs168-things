...
class FlowTableTest(unittest.TestCase):
    def test_remove_matching_entries(self):
        ...
        for match, priority, strict, remaining in (
            ...
            (
                ...
                True,
                [1, 2, 3],
            ),  # strict wildcard with wrong prio removes nothing
            (
                ofp_match(),
                1,
                True,
                [1, 2],
            ),  # strict wildcard with right prio removes only flow 3
            (
                ofp_match(nw_src="1.2.3.0/24"),
                1,
                False,
                [3],
            ),  # non-strict subnet removes 1+2
            (
                ofp_match(nw_src="1.2.3.0/24"),
                6,
                True,
                [1, 2, 3],
            ),  # does not match dl_src => noop
            (
                ofp_match(dl_src=EthAddr("00:00:00:00:00:02"), nw_src="1.2.3.0/24"),
                5,
                True,
                [1, 3],
            ),  # exactly matches #2
    ...
    def test_remove_expired_entries(self):
        ...
        for cookie, idle, hard in ((1, 5, 20), (2, 5, 20), (3, 0, 20), (4, 0, 0)):
            t.add_entry(
                TableEntry(now=0, cookie=cookie, idle_timeout=idle, hard_timeout=hard)
            )
