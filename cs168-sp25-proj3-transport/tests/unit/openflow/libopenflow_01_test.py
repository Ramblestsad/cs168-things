...
class ofp_match_test(unittest.TestCase):
    def test_bit_wildcards(self):
        ...
        for k, v in ofp_match_data.items():
            self.assertEquals(
                getattr(m, k),
                None,
                "Attr %s should be wildcarded and reported as None" % k,
            )
            self.assertEquals(m.wildcards & v[1], v[1])

        # try setting and unsetting specific bit-level match entries
        for change in [
            ("in_port", 1, OFPFW_IN_PORT),
            ("dl_vlan", 2, OFPFW_DL_VLAN),
            ("tp_dst", 22, OFPFW_TP_DST),
        ]:
            setattr(m, change[0], change[1])
            self.assertEquals(
                getattr(m, change[0]),
                change[1],
                "Attr %s should have been set to %s" % change[0:2],
            )
            self.assertEquals(
                m.wildcards & change[2],
                0,
                "with %s set to %s, wildcard bit %x should get unset" % change,
            )
            setattr(m, change[0], None)
            self.assertEquals(
                m.wildcards & change[2],
                change[2],
                "with %s reset from %s, wildcard bit %x should be set again" % change,
            )
    ...
    def test_ip_wildcard_magic(self):
        ...
        for attr, bitmask, shift in (
            ("nw_src", OFPFW_NW_SRC_MASK, OFPFW_NW_SRC_SHIFT),
            ("nw_dst", OFPFW_NW_DST_MASK, OFPFW_NW_DST_SHIFT),
        ):
            m = ofp_match()
            self.assertEquals(
                getattr(m, "get_" + attr)(),
                (None, 0),
                "get_%s for unset %s should return (None,0)" % (attr, attr),
            )
    ...
    def test_match_with_wildcards(self):
        ...
        def assertMatch(ref, other, msg=""):
            self.assertTrue(
                ref.matches_with_wildcards(other),
                "%s - %s should match %s " % (msg, ref.show(), other.show()),
            )

        def assertNoMatch(ref, other, msg=""):
            self.assertFalse(
                ref.matches_with_wildcards(other),
                "%s - %s should NOT match %s " % (msg, ref.show(), other.show()),
            )
        ...
        for changes in (
            {"in_port": 15},
            {"dl_src": "12:34:56:78:90:ab", "dl_vlan": 7},
            {"tp_dst": 22},
        ):
            wild = create()
            concrete = create()
...
class ofp_command_test(unittest.TestCase):
    ...
    def assert_packed_header(self, pack, ofp_type, length, xid):
        ...
        def assert_num(name, start, length, expected):
            ...
            self.assertEquals(
                val,
                expected,
                "packed header check: %s for ofp type %s should be %d (is %d)"
                % (name, ofp_type_map[ofp_type], expected, val),
            )
    ...
    def _test_pack_unpack(self, o, xid, ofp_type=None):
        ...
        self.assertTrue(
            o._assert(),
            "pack_unpack for %s -- original object should _assert to true" % show(o),
        )
        ...
        self.assertEqual(
            len(o),
            len(pack),
            "pack_unpack for %s -- len(object)=%d != len(packed)=%d"
            % (type(o), len(o), len(pack)),
        )
        ...
        self.assertEqual(
            o,
            unpacked,
            "pack_unpacked -- original != unpacked\n===Original:\n%s\n===Repacked:%s\n"
    ...
    def test_header_pack_unpack(self):
        for kw in (
            {"header_type": OFPT_PACKET_OUT, "xid": 1},
    ...
    some_actions = (
        [],
        [out(port=2)],
        [out(port=2), out(port=3)],
        [out(port=OFPP_FLOOD)],
        [dl_addr.set_dst(EthAddr("00:" * 5 + "01")), out(port=1)],
    )
    ...
    def test_pack_flow_mod_openflow_dl_type_wildcards(self):
        ...
        def test_wildcards(match, expected):
            ...
            self.assertEquals(
                ...
                expected,
                "packed: %s <> expected: %s"
                % (show_wildcards(packed), show_wildcards(expected)),
            )
        ...
        all_normalized = (
            (OFPFW_ALL & ~(OFPFW_NW_SRC_MASK | OFPFW_NW_DST_MASK))
            | OFPFW_NW_SRC_ALL
            | OFPFW_NW_DST_ALL
        )
        ...
        test_wildcards(
            ofp_match(dl_type=0x806),
            all_normalized
            & ~(OFPFW_NW_TOS | OFPFW_TP_SRC | OFPFW_TP_DST | OFPFW_DL_TYPE),
        )
        # dl type = IP -> more wildcards live
        test_wildcards(
            ofp_match(dl_type=0x800),
            all_normalized & ~(OFPFW_TP_SRC | OFPFW_TP_DST | OFPFW_DL_TYPE),
        )
        # dl type = IP, nw_proto=UDP -> alll wildcards live
        test_wildcards(
            ofp_match(dl_type=0x800, nw_proto=6),
            all_normalized & ~(OFPFW_DL_TYPE | OFPFW_NW_PROTO),
        )
...
class ofp_action_test(unittest.TestCase):
    def assert_packed_action(self, cls, packed, a_type, length):
        self.assertEqual(
            extract_num(packed, 0, 2),
            a_type,
            "Action %s: expected type %d (but is %d)"
            % (cls, a_type, extract_num(packed, 0, 2)),
        )
        self.assertEqual(
            extract_num(packed, 2, 2),
            length,
            "Action %s: expected length %d (but is %d)"
            % (cls, length, extract_num(packed, 2, 2)),
        )
    ...
    def test_pack_all_actions_simple(self):
        ...
        p = c(
            ofp_action_dl_addr.set_dst,
            OFPAT_SET_DL_DST,
            {"dl_addr": EthAddr("01:02:03:04:05:06").toRaw()},
            16,
        )
        self.assertEquals(extract_num(p, 4, 6), 0x010203040506)
        p = c(
            ofp_action_dl_addr.set_src,
            OFPAT_SET_DL_SRC,
            {"dl_addr": EthAddr("ff:ee:dd:cc:bb:aa").toRaw()},
            16,
        )
        self.assertEquals(
            extract_num(p, 4, 6),
            0xFFEEDDCCBBAA,
            "Ethernet in packed is %x, but should be ff:ee:dd:cc:bb:aa"
            % extract_num(p, 4, 6),
        )
        p = c(
            ofp_action_nw_addr.set_dst,
            OFPAT_SET_NW_DST,
            {"nw_addr": IPAddr("1.2.3.4")},
            8,
        )
        self.assertEquals(extract_num(p, 4, 4), 0x01020304)
        p = c(
            ofp_action_nw_addr.set_src,
            OFPAT_SET_NW_SRC,
            {"nw_addr": IPAddr("127.0.0.1")},
            8,
        )
